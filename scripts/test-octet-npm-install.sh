...
[[ "$(HOME="$home" octet --version)" == "octet $version" ]]
HOME="$home" octet --help >/dev/null
frame=$(HOME="$home" octet-host)
[[ "$(printf '%s\n' "$frame" | awk 'END { print NR }')" == 1 ]]
printf '%s\n' "$frame" | grep -F '"type":"hello"' >/dev/null

probe_output=$(cd "$probe_directory" && HOME="$home" OCTET_NPM_TEST_ENV=preserved octet --probe argv-value)
...
[[ "$(HOME="$home" "$work_directory/octet-symlink" --version)" == "octet $version" ]]
