...
Build the pinned linux/amd64 octet image (default: octet:0.7.4), or materialize
...
if [[ "$mode" == build ]] && ! command -v docker >/dev/null 2>&1; then
    printf 'Docker is required to build the octet image\n' >&2
    exit 1
fi
...
if ! git -C "$repository_directory" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    printf 'octet container source must be a Git checkout\n' >&2
    exit 1
fi
git -C "$repository_directory" update-index -q --refresh
if ! git -C "$repository_directory" diff-index --quiet HEAD --; then
    printf 'octet container source has tracked changes; build an immutable clean commit\n' >&2
    exit 1
fi
source_commit=$(git -C "$repository_directory" rev-parse 'HEAD^{commit}')
if [[ ! "$source_commit" =~ ^[0-9a-f]{40}$ ]]; then
    printf 'could not resolve an immutable octet source commit\n' >&2
    exit 1
fi

index_manifest=$(mktemp "${TMPDIR:-/tmp}/octet-container-index.XXXXXX")
temporary_context=
cleanup() {
    rm -f "$index_manifest"
...
if [[ "$mode" == context ]]; then
    ...
    temporary_context=$(mktemp -d "${TMPDIR:-/tmp}/octet-container-context.XXXXXX")
...
if [[ "$mode" == context ]]; then
    ...
    docker build \
        --file "$context/deploy/Dockerfile.octet" \
        --tag "$image" \
        "$context"
fi
