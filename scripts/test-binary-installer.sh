...
case "$request" in
    *'"request_id":"installer-probe"'*)
        printf '%s\\n' '{"protocol_version":1,"request_id":"installer-probe","seq":1,"type":"hello","data":{"sdk_version":"0.7.2"}}'
        ;;
    *) exit 2 ;;
esac
''',
...
PY
    {
        ...
        printf '%064d  ./octet-0.7.2-x86_64-apple-darwin.tar.gz\n' 0
        printf '%064d  ./octet-0.7.2-x86_64-unknown-linux-gnu.tar.gz\n' 0
...
if [ "${OCTET_TEST_HARDLINK_ARCHIVE:-0}" = 1 ] && [ "$name" = octet-0.7.2-aarch64-apple-darwin.tar.gz ]; then
    ln "$source" "$output"
else
    cp "$source" "$output"
fi
if [ "${OCTET_TEST_TAMPER_ARCHIVE:-0}" = 1 ] && [ "$name" = octet-0.7.2-aarch64-apple-darwin.tar.gz ]; then
...
printf '%s\n' '{"protocol_version":1,"request_id":"installer-probe","command":"hello"}' \
    ...
    | grep -F '"sdk_version":"0.7.2"' >/dev/null
test "$("$positive_home/bin/octet" --version)" = 'octet 0.7.2'
...
test "$("$upgrade_home/bin/octet" --version)" = 'octet 0.7.2'
printf '%s\n' '{"protocol_version":1,"request_id":"installer-probe","command":"hello"}' \
...
test "$("$legacy_home/bin/octet" --version)" = 'octet 0.7.2'
