...
if [ "$#" -ne 1 ]; then
    echo "usage: scripts/smoke-octet-serve-installed.sh /absolute/path/to/octet" >&2
    exit 2
fi
...
if [ ! -x "$binary" ]; then
    echo "installed octet binary is not executable: $binary" >&2
    exit 2
fi

script_directory=$(cd "$(dirname "$0")" && pwd)
repository_directory=$(cd "$script_directory/.." && pwd)
expected_directory="$repository_directory/extensions/octet-serve/web"
work_directory=$(mktemp -d "${TMPDIR:-/tmp}/octet-serve-smoke.XXXXXX")
...
(
    ...
    exec env -i \
        ...
        OPENAI_API_KEY=octet-serve-smoke-not-a-secret \
        OCTET_MODEL=gpt-5.4 \
        OCTET_SESSION_DIR="$work_directory/sessions" \
        OCTET_OFFLINE=true \
        OCTET_EXTENSIONS= \
        OCTET_TRUSTED_EXTENSIONS= \
...
while [ "$attempt" -lt 100 ]; do
    launch_url=$(sed -n 's/^Open octet once: //p' "$server_log" | sed -n '1p')
    if [ -n "$launch_url" ]; then
        break
    fi
    if ! kill -0 "$server_pid" >/dev/null 2>&1; then
        echo "installed octet exited before publishing its launch URL" >&2
        sed -E 's#(/__octet/launch/)[0-9a-f]{64}#\1<redacted>#g' "$server_log" >&2
        exit 1
    fi
    attempt=$((attempt + 1))
    sleep 0.1
...
if [ -z "$launch_url" ]; then
    echo "installed octet did not publish a launch URL in time" >&2
    exit 1
fi
if ! printf '%s\n' "$launch_url" \
    | grep -Eq '^http://127\.0\.0\.1:[0-9]+/__octet/launch/[0-9a-f]{64}$'; then
    echo "installed octet published a malformed or non-loopback launch URL" >&2
    exit 1
fi

origin=${launch_url%%/__octet/launch/*}
curl -fsS \
    --noproxy '*' \
    --proto '=http' \
    --connect-timeout 2 \
...
do
    ...
    grep -Fxi "x-octet-web-bundle: $bundle_sha256" "$headers" >/dev/null
done

if grep -Fq "graphical shell is not bundled" "$work_directory/download/index.html"; then
    echo "installed octet served the retired placeholder shell" >&2
    exit 1
fi

printf 'installed octet served the checked web bundle %s\n' "$bundle_sha256"
