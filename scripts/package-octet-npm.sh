...
usage() {
    printf 'usage: %s VERSION NATIVE_ARCHIVE_DIRECTORY OUTPUT_DIRECTORY OCTET_SHA256SUMS\n' "$0" >&2
...
work_directory=$(mktemp -d "${TMPDIR:-/tmp}/octet-npm.XXXXXX")
...
def validate_archive(target, archive, expected_digest):
    ...
    root_name = f"octet-{version}-{target}"
...
python3 "$script_directory/verify-octet-npm.py" "$version" "$output_directory"
