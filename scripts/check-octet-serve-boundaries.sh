...
if ! git rev-parse --verify "${base_ref}^{commit}" >/dev/null 2>&1; then
  echo "unknown octet serve boundary base: ${base_ref}" >&2
  exit 2
fi
if ! git merge-base --is-ancestor "$base_ref" HEAD; then
  echo "octet serve boundary base is not an ancestor of HEAD: ${base_ref}" >&2
  exit 2
fi
...
for changed_file in $changed_files; do
  case "$changed_file" in
    apps/web/* \
      | extensions/octet-serve/* \
      | docs/experimental/octet-serve/* \
      | docs/design/config-diagnostics.md \
      | docs/design/serve-lifecycle-safety.md \
      | SECURITY.md \
      | scripts/check-octet-serve-boundaries.sh \
      | scripts/package-octet-serve-release.sh \
      | scripts/smoke-octet-serve-installed.sh \
      ...
      | .gitignore)
      ;;
    crates/octet-coding-agent/Cargo.toml \
      | crates/octet-coding-agent/src/cli.rs \
      | crates/octet-coding-agent/src/extension_package.rs \
      | crates/octet-coding-agent/src/main.rs \
      | crates/octet-coding-agent/src/commands.rs \
    ...
    crates/octet-agent/src/agent.rs \
      | crates/octet-agent/src/context.rs \
      | crates/octet-agent/src/lib.rs \
    ...
    crates/octet-coding-agent/src/modes/rpc.rs \
      | crates/octet-coding-agent/src/config.rs \
      | crates/octet-coding-agent/src/resource_resolver.rs \
      | crates/octet-coding-agent/src/resources.rs \
      | crates/octet-coding-agent/src/session_store.rs)
      ;;
    *)
...
if [ -n "$violations" ]; then
  echo "octet serve crossed its optional extension boundary:" >&2
  printf '%s' "$violations" >&2
  exit 1
fi

echo "octet serve changes stay within the optional extension/application boundary"
