...
if [[ ! "$version" =~ ^v[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?$ ]]; then
    printf 'version must be a canonical octet version tag such as v0.7.0: %s\n' "$version" >&2
    exit 2
fi
...
case "$target" in
    ...
    *)
        printf 'unsupported octet-serve release target: %s\n' "$target" >&2
        exit 2
        ;;
esac
...
binary="$repository_directory/target/$target/release/octet"
package_version=${version#v}
artifact_name="octet-serve-${package_version}-${target}"
staging_directory=$(mktemp -d "${TMPDIR:-/tmp}/octet-serve-release.XXXXXX")
package_directory="$staging_directory/octet-serve"
trap 'rm -rf "$staging_directory"' EXIT

if [[ ! -f "$binary" || -L "$binary" || ! -x "$binary" ]]; then
    printf 'release binary is missing, linked, or not executable: %s\n' "$binary" >&2
    printf 'build it with: cargo build --release --locked --target %s -p octet-coding-agent --features serve\n' "$target" >&2
    exit 1
fi
...
cp "$binary" "$package_directory/bin/octet-serve-runtime"
chmod 0755 "$package_directory/bin/octet-serve-runtime"
binary_sha256=$(sha256_file "$package_directory/bin/octet-serve-runtime")

cat >"$package_directory/package.toml" <<EOF
schema_version = 1
id = "octet-serve"
version = "$package_version"
requires_octet = "=$package_version"
target = "$target"
    ...
    with gzip.GzipFile(filename="", mode="wb", fileobj=raw, compresslevel=9, mtime=epoch) as compressed:
        with tarfile.open(fileobj=compressed, mode="w", format=tarfile.GNU_FORMAT) as output:
            for path in paths:
                ...
                name = "octet-serve" if not relative.parts else f"octet-serve/{relative.as_posix()}"
...
expected = ["octet-serve", "octet-serve/bin", "octet-serve/bin/octet-serve-runtime", "octet-serve/package.toml"]
