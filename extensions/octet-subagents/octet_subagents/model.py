"""Bounded policy and state models for the octet-subagents orchestrator."""
...
class SpawnRequest:
    ...
    def child_message(self, owner: Owner) -> str:
        ...
        message = f"""[octet bounded subagent policy v1]
...
- The workspace/cwd, environment, sandbox, approval policy, extensions, and filesystem are inherited from the parent octet session. A shared cwd is not isolation. Host policy remains authoritative.
...
COMPLETION
Return one concise final answer with: Summary, Evidence (stable file locations when applicable), Uncertainty, and Artifact/session references exposed by octet. Do not stream prose to the parent or infer completion from prose.
