"""Executable API 0.2 protocol wiring for octet-subagents."""
...
def _require_agent_sessions(extension: Extension) -> None:
    ...
    if "agent_sessions" in missing:
        message = (
            "the trusted octet-subagents extension does not have an active owner-bound API 0.2 "
            "agent_sessions service; missing: %s"
        ) % ", ".join(missing)
    else:
        message = (
            "the trusted octet-subagents extension does not have the required delegation telemetry "
            "contract `%s`; restart octet after reinstalling a matching current host and "
            "octet-subagents bundle"
...
def _compact_metadata(result: Mapping[str, Any]) -> Dict[str, Any]:
    metadata: Dict[str, Any] = {
        "schema": "octet.subagents.result.v1",
...
def _result_text(operation: str, result: Mapping[str, Any]) -> str:
    ...
    if operation == "spawn":
        ...
        if operation == "wait" and result.get("wait_timed_out"):
            ...
            lines.append("Completion summaries are delivered through octet's host-owned claim/ack parent-turn boundary.")
...
def _error_result(operation: str, error: Exception) -> Dict[str, Any]:
    ...
    return tool_result(
        ...
        is_error=True,
        metadata={
            "schema": "octet.subagents.error.v1",
            "operation": operation,
            "code": code,
        },
    )
...
def create_runtime() -> tuple[Extension, Orchestrator, PresentationPublisher]:
    ...
    @extension.tool(
        name="subagent_wait",
        description=(
            "Wait at most 60 seconds for one or all owned workers, while preserving background execution on wait cancellation/expiry and settling an observed worker wall deadline through octet cancellation."
        ),
