...
9. The coding TUI updates one bounded owner-fenced **Subagents** transcript block in place, including between root turns. Its state-count heading and up to four active child lines show task and input/output tokens; `ctrl+o` retains disclosure. `/subagents` exposes the complete roster, model, tool-call counts, cost, and reasons. octet mirrors settled child usage into the root session ledger, so child spend contributes exactly once to the cumulative footer.
