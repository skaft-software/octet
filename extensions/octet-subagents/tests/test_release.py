...
class ReleaseTests(unittest.TestCase):
    ...
    def test_bundle_is_self_contained_and_vendored_sdk_is_synchronized(self):
        required = (
            "octet-subagents",
            "octet_subagents/runtime.py",
            "octet_subagents/orchestrator.py",
            "octet_subagents/presentation.py",
            "vendor/octet_extension/__init__.py",
            "vendor/octet_extension/api_v03.py",
            "vendor/octet_extension/extension.py",
            "vendor/octet_extension/protocol.py",
            "skills/octet-subagents/SKILL.md",
        ...
        vendored = ROOT / "vendor" / "octet_extension"
    ...
    def test_host_staged_entrypoint_release_handshake_and_shutdown_smoke(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            staged_entrypoint = Path(directory) / "octet-subagents"
            staged_entrypoint.write_bytes((ROOT / "octet-subagents").read_bytes())
            staged_entrypoint.chmod(0o755)
            environment = os.environ.copy()
            environment["OCTET_EXTENSION_DIR"] = str(ROOT)
    ...
    def test_release_archive_has_one_regular_portable_root(self):
        with tempfile.TemporaryDirectory() as directory:
            archive = Path(directory) / "octet-subagents-0.7.0.tar.gz"
            ...
            self.assertEqual({member.name.split("/", 1)[0] for member in members}, {"octet-subagents"})
    ...
    def test_installed_bundle_regression_fixture_preserves_authoritative_failure_evidence(self):
        ...
        self.assertEqual(fixture["schema"], "octet.subagents.installed-bundle-regression.v1")
    ...
    def test_packaged_skill_documents_default_read_only_scope_granted_writers_and_non_recursive(self):
        skill = (ROOT / "skills" / "octet-subagents" / "SKILL.md").read_text(encoding="utf-8")
