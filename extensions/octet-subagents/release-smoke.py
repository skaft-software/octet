#!/usr/bin/env python3
"""Compare one direct run with one bounded octet-subagents run.

The recipe deliberately consumes measurements captured by the caller instead of
starting octet or a provider itself. That keeps release smoke deterministic and
...
def compare(direct: Mapping[str, Any], delegated: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "schema": "octet.subagents.release-smoke.v1",
