...
PROFILE_SCHEMA = "octet.browse.profile.v1"
SENTINEL_NAME = ".octet-browse-profile.json"
...
class ProfileManager:
    ...
    def acquire(self, *, create: bool) -> ProfileLease:
        ...
        if not lock.acquire():
            raise BrowseError(
                "profile_locked",
                "Another octet Browse process owns the isolated profile.",
            )
        try:
            path = self.paths.profile
    ...
    def reset(self) -> bool:
        """Delete only a locked, sentinel-verified octet Browse profile."""
    ...
    def _create_profile(self, path: Path) -> None:
        ...
        try:
            ...
            if not stat.S_ISDIR(metadata.st_mode) or stat.S_ISLNK(metadata.st_mode):
                raise BrowseError(
                    "unsafe_profile", "The octet Browse profile path must be a real directory."
                )
    ...
    def _validate_profile(path: Path) -> None:
        try:
            ...
            raise BrowseError("profile_missing", "The octet Browse profile is missing.") from error
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode):
            raise BrowseError(
                "unsafe_profile", "The octet Browse profile path must be a non-symlink directory."
            )
        sentinel = path / SENTINEL_NAME
        try:
