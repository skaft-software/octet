...
class SetupManager:
    ...
    def status(self) -> SetupStatus:
        ...
        if self._lock_is_held_elsewhere():
            return SetupStatus("installing", "Pinned browser setup is running in another octet process.", log)
    ...
    def start(self) -> SetupStatus:
        ...
        with self._mutex:
            ...
            if not lock.acquire():
                return SetupStatus(
                    "installing",
                    "Pinned browser setup is running in another octet process.",
    ...
    def validate_runtime(self, runtime: Optional[Path] = None) -> None:
        ...
        for owned_directory, label in (
            (self.paths.root, "octet Browse root"),
            (self.paths.runtime_parent, "octet Browse runtime parent"),
        ):
            try:
        ...
        if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode):
            raise BrowseError("unsafe_runtime", "The octet Browse runtime must be a non-symlink directory.")
    ...
    def _install_background(self, lock: ExclusiveFileLock) -> None:
        ...
        try:
            ...
            with self._open_install_log(append=False) as log:
                log.write(f"octet Browse setup: playwright=={PLAYWRIGHT_VERSION}\n")
                log.flush()
    ...
    def _read_bounded_json(path: Path) -> Any:
        try:
            ...
            raise BrowseError("missing_file", "An expected octet Browse state file is missing.") from error
        if (
            ...
            raise BrowseError("unsafe_state", "A octet Browse state file is invalid.")
        flags = os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0)
        try:
            ...
            try:
                ...
                if (
                    ...
                    raise BrowseError("unsafe_state", "A octet Browse state file changed during validation.")
                raw = os.read(fd, MAX_STATE_BYTES + 1)
            finally:
                os.close(fd)
            return json.loads(raw.decode("utf-8"))
        except (OSError, UnicodeError, json.JSONDecodeError) as error:
            raise BrowseError("unsafe_state", "A octet Browse state file is invalid.") from error
    ...
    def _write_json(path: Path, value: Mapping[str, Any]) -> None:
        ...
        if len(payload) > MAX_STATE_BYTES:
            raise BrowseError("state_too_large", "A octet Browse state record exceeded its bound.")
