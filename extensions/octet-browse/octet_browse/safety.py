...
try:
    import fcntl
except ImportError:  # pragma: no cover - octet currently targets Unix hosts.
...
class ExclusiveFileLock:
    ...
    def acquire(self) -> bool:
        ...
        if fcntl is None:
            raise BrowseError(
                "locking_unavailable",
                "octet Browse requires Unix advisory file locking.",
            )
        ...
        try:
            ...
            if error.errno in {errno.ELOOP, errno.EMLINK}:
                raise BrowseError(
                    "unsafe_lock_path", "The octet Browse lock path is a symbolic link."
                ) from error
            raise BrowseError("lock_failed", "The octet Browse lock could not be opened.") from error
        try:
            metadata = os.fstat(fd)
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
                raise BrowseError(
                    "unsafe_lock_path", "The octet Browse lock must be a regular owned file."
                )
            try:
                os.fchmod(fd, 0o600)
            except OSError:
    ...
    def __enter__(self) -> "ExclusiveFileLock":
        if not self.acquire():
            raise BrowseError("lock_held", "Another octet Browse owner holds the lock.")
        return self
