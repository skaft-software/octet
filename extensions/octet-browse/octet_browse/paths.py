"""Filesystem locations owned exclusively by octet Browse."""
...
class BrowsePaths:
    """Resolved octet-owned state paths.
    ...
    def for_home(cls, home: Optional[Path] = None) -> "BrowsePaths":
        ...
        return cls(base.expanduser().absolute() / ".octet" / "browse")
    ...
    def ensure_root(self) -> None:
        ...
        _require_directory(parent, "octet state parent")
        if self.root.exists() or self.root.is_symlink():
            _require_directory(self.root, "octet Browse root")
        else:
    ...
    def ensure_directory(self, path: Path) -> None:
        ...
        try:
            ...
            raise PathSafetyError("owned directory escaped the octet Browse root") from error
        current = self.root
