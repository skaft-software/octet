"""API 0.2 tool and command wiring for the official octet Browse bundle."""
...
TOOL_DESCRIPTIONS = {
    ...
    "browser_screenshot": "Capture only the current viewport when no tool-typed value or visible form/editable field could leak, retain it under octet-owned screenshot storage below 5 MiB, and publish an owner/generation-scoped API 0.2 artifact plus a read-compatible textual path.",
...
def create_runtime(
    *, controller_factory: Optional[Any] = None
        ...
        try:
            ...
            if name == "browser_status":
                ...
                return tool_result(
                    ...
                    metadata={
                        "schema": "octet.browse.screenshot.v1",
                        "operation": name,
                        "tab_id": tab_id,
    ...
    @extension.command(
        name="browse",
        description="Set up, inspect, open, close, or safely reset the visible isolated octet Browse browser.",
    ...
    def browse_status_surface(_params: Mapping[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "style_role": "extension.octet_browse.status",
            "priority": 20,
        }
...
def _tab(values: Mapping[str, Any]) -> str:
    ...
    if not valid_tab_id(value):
        raise BrowseError("invalid_tab", "tab_id must be an opaque ID returned by octet Browse.")
    return value
