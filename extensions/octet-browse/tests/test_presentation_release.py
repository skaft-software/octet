...
from octet_browse.presentation import BrowsePresentation, PresentationPublisher
        ...
        model.update_setup(
            {
                ...
                "log_path": "~/.octet/browse/install.log",
            }
        )
...
class PackageTests(unittest.TestCase):
    def test_manifest_surface_metadata_and_executable(self) -> None:
        ...
        self.assertEqual(manifest["requires_octet"], "=0.7.0")
        ...
        worker_source = (PACKAGE / "octet_browse" / "worker.py").read_text(encoding="utf-8")
    ...
    def test_vendored_sdk_is_byte_synchronized_and_has_revision_guard(self) -> None:
        shared = REPOSITORY / "sdk" / "python" / "octet_extension"
        vendor = PACKAGE / "vendor" / "octet_extension"
        ...
        with tempfile.TemporaryDirectory() as home:
            ...
            process = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    "import pathlib,sys,threading,octet_browse.runtime; "
                    "assert 'playwright' not in sys.modules; "
                    "assert [t.name for t in threading.enumerate()] == ['MainThread']; "
                    "assert not (pathlib.Path.home()/'.octet'/'browse').exists()",
                ],
                env=environment,
    ...
    def test_entrypoint_handshake_and_shutdown_do_not_install_or_launch(self) -> None:
        with tempfile.TemporaryDirectory() as home:
            ...
            environment["HOME"] = home
            environment["PYTHONDONTWRITEBYTECODE"] = "1"
            environment["OCTET_EXTENSION_DIR"] = str(PACKAGE)
    ...
    def test_local_release_packaging_smoke_contains_runtime_skill_and_fixtures(self) -> None:
        script = REPOSITORY / "scripts" / "package-octet-extension-release.sh"
        if not script.is_file():
            self.skipTest("generic extension release packager is not present")
        with tempfile.TemporaryDirectory() as output:
            process = subprocess.run(
                [str(script), "octet-browse", output, "v0.7.0", str(PACKAGE)],
                cwd=REPOSITORY,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                text=True,
            ...
            self.assertEqual(process.returncode, 0, process.stderr)
            archive = Path(output) / "octet-browse-0.7.0.tar.gz"
            self.assertTrue(archive.is_file())
            with tarfile.open(archive, "r:gz") as bundle:
                names = set(bundle.getnames())
            required = {
                "octet-browse/extension.toml",
                "octet-browse/extension.py",
                "octet-browse/octet_browse/runtime.py",
                "octet-browse/octet_browse/worker.py",
                "octet-browse/vendor/octet_extension/extension.py",
                "octet-browse/skills/octet-browse/SKILL.md",
                "octet-browse/presentation-fixtures/installing.json",
                "octet-browse/presentation-fixtures/stale-generation-cleared.json",
                "octet-browse/README.md",
                "octet-browse/LICENSE",
            }
