...
/**
 * octet's deliberately small Pi compatibility host.
 *
 ...
 * surfaces onto octet's API 0.2 JSON-RPC bus.
 */
...
const API_0_3_SCHEMA = "octet.extension.api/0.3";
const API_0_3_ENCODING = "octet-canonical-json-v1";
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === "--extension" || value === "-e") {
      ...
      index += 1;
    } else if (value === "--octet-version") {
      result.octetVersion = requiredValue(value, index);
      index += 1;
  ...
  if (result.extensions.length === 0 && process.env.OCTET_PI_EXTENSION) {
    result.extensions.push(process.env.OCTET_PI_EXTENSION);
  }
  ...
  if (result.strictIdentity) {
    if (
      ...
      throw new Error(
        "pinned Pi aggregate identity requires --pi-package, per-source source/lock fingerprints, runtime integrity, aggregate digest, link manifest, link identity, and octet version",
      );
    }
  }
  return result;
...
function parentRequestId() {
  ...
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0) {
    throw new Error("Pi compatibility host has no active octet request owner");
  }
  return value;
}
...
function makeExtensionActions() {
  return {
    ...
    setModel: () => Promise.reject(new Error("Pi compatibility API is not supported by octet: pi.setModel")),
...
const PI_PROVIDER_CONFIG_FIELDS = new Set(["name", "api", "models", "auth", "octetAuth", "octetStream"]);
...
function normalizePiProvider(name, config) {
  ...
  if (source.auth !== undefined && source.octetAuth !== undefined) {
    throw providerError(`provider ${providerId} config cannot declare both auth and octetAuth`);
  }
  ...
  const auth = normalizeProviderAuth(source.auth ?? source.octetAuth);
  ...
  if (source.octetStream !== undefined && typeof source.octetStream !== "function") {
    throw providerError(`provider ${providerId} octetStream must be a function`);
  }
  return {
    provider: {
      id: providerId,
...
function registerPiProvider(name, config) {
  try {
    ...
    if (typeof normalized.adapter !== "function") {
      throw providerError(`provider ${normalized.provider.id} requires a secret-free octetStream adapter`);
    }
...
function asyncIteratorForProvider(value) {
  if (!value || typeof value[Symbol.asyncIterator] !== "function") {
    throw providerError("octetStream must return an AsyncIterable of Pi assistant-message events");
  }
...
function normalizeHostAdapterEvent(kind, payload) {
  // A custom octetStream adapter may use the API 0.3 event shape directly, but
...
function fingerprintSource(source) {
  ...
  hash.update(Buffer.from("octet-pi-source-fingerprint\0"));
...
function fingerprintSourceLocks(source) {
  ...
  const hash = createHash("sha256");
  hash.update(Buffer.from("octet-pi-source-lock-fingerprint\0"));
...
function piRuntimeIntegrity(root) {
  ...
  const hash = createHash("sha256");
  hash.update(Buffer.from("octet-pi-runtime-integrity\0"));
...
function calculateLinkIdentity(piRuntime) {
  const hash = createHash("sha256");
  hash.update(Buffer.from("octet-pi-aggregate-link-identity\0"));
...
function verifyRuntimeIdentity(piRuntime, params) {
  ...
  if (
    ...
    || params.octet_version !== args.octetVersion
  ) {
...
function findPiPackageRoot(extensionPaths, selectedPackage) {
  ...
  for (const [name, value] of [
    ["OCTET_PI_PACKAGE", process.env.OCTET_PI_PACKAGE],
  ...
  throw new Error(
    `could not locate ${SUPPORTED_PI_PACKAGE}@${SUPPORTED_PI_VERSION}; set OCTET_PI_PACKAGE to its package root.${suffix}`,
  );
}

function commandDefinitionToOctet(command) {
...
function sameToolDefinition(left, right) {
  return JSON.stringify(toolDefinitionToOctet(left.definition)) === JSON.stringify(toolDefinitionToOctet(right.definition));
}
      ...
      if (!bridge.features.has("artifacts")) {
        appendLoweredContent(lowered, {
          ...
          text: part.alt
            ...
            : "[Pi extension returned an image, but octet artifact support was not negotiated]",
        });
        continue;
      }
...
function assertV03InitializeParams(params) {
  ...
  for (const name of ["api_version", "octet_version", "workspace"]) {
...
async function loadBridge(params, v03Selection = undefined) {
  ...
  bridge = {
    ...
    agentDir: resolve(args.agentDir ?? process.env.OCTET_PI_AGENT_DIR ?? join(homedir(), ".pi/agent")),
  ...
  for (const [index, extension] of loaded.extensions.entries()) {
    ...
    // untrusted source merely to synthesize octet's pre-start API 0.3 manifest flags.
...
async function handleInitialize(message) {
  ...
  const tools = isApiV03()
    ...
    : bridge.tools.map((tool) => toolDefinitionToOctet(tool.definition));
  if (isApiV03()) {
  ...
  const commands = bridge.features.has("runtime_commands")
    ? bridge.commands.map(commandDefinitionToOctet)
    : [
        {
          name: bridge.commandName,
  ...
  for (const warning of bridge.unsupported) diagnostic(`Pi compatibility: ${warning} is unavailable in octet`);
  diagnostic(
...
async function runHook(message) {
  ...
  if (hook === "before_tool_call") {
    const toolCallId = `pi-octet-hook-${String(message.id)}`;
  ...
  if (hook === "after_tool_call") {
    if (!bridge.toolNames.includes(payload.name)) {
      const transformed = await bridge.runner.emitToolResult({
        type: "tool_result",
        toolCallId: `pi-octet-hook-${String(message.id)}`,
        toolName: payload.name,
      ...
      if (transformed !== undefined) {
        return unsupported("tool_result mutation for octet-native tools");
      }
    }
  ...
  throw new Error(`unsupported octet hook ${hook}`);
}
...
async function handleLifecycle(method, params) {
  if (method === "session/started") {
    ...
    const hostId = params.tool_call_id ?? "octet-tool";
    let piId = hostId;
...
async function onMessage(message) {
  ...
  if (message.id !== undefined && !message.method) {
    ...
    if (!pending) return;
    pendingHostRequests.delete(message.id);
    if (message.error) pending.reject(new Error(message.error.message ?? "octet host request failed"));
