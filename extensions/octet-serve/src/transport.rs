...
const X_OCTET_WEB_BUNDLE: HeaderName = HeaderName::from_static("x-octet-web-bundle");
const X_OCTET_GOAL_REVISION: HeaderName = HeaderName::from_static("x-octet-goal-revision");
...
impl LoopbackServer {
    ...
    pub async fn start<H: HostService>(
        ...
        let auth = TransportAuth {
            ...
            cookie_name: format!("octet_{}", random_hex(12)?),
    ...
    pub fn launch_url(&self) -> String {
        format!(
            "http://{}/__octet/launch/{}",
            self.address, self.launch_token
        )
    }
...
fn build_router<H: HostService>(state: Arc<TransportState<H>>) -> Router {
    Router::new()
        .route("/", get(index::<H>))
        .route("/__octet/launch/{token}", get(exchange_launch_token::<H>))
...
async fn static_asset<H: HostService>(
    ...
    if asset.starts_with("api/") || asset.starts_with("__octet/") {
...
fn web_asset_response(bundle: &WebBundle, path: &str) -> Response {
    ...
    (
        [
            ...
            (X_OCTET_WEB_BUNDLE, bundle_sha256),
        ],
        asset.bytes,
    )
        .into_response()
...
fn session_export_content_disposition(
    ...
    HeaderValue::from_str(&format!(
        "attachment; filename=\"octet-session-{}.json\"",
        session_id.as_str()
    ))
}
...
mod tests {
            ...
            match session_id.as_str() {
                ...
                "blocked-export" => {
                    ...
                    Ok(bytes::Bytes::from_static(
                        br#"{"format":"octet-session-export","redacted":true}"#,
                    ))
                }
    ...
    fn exchange_request(address: SocketAddr, token: &str) -> String {
        get_request(address, &format!("/__octet/launch/{token}"))
    }
    ...
    async fn goal_transport_supports_lifecycle_and_restart_persistence() {
        ...
        assert_eq!(
            response_header(&created, "x-octet-goal-revision"),
            Some("1")
        );
        ...
        assert!(restored.starts_with("HTTP/1.1 200"));
        assert_eq!(
            response_header(&restored, "x-octet-goal-revision"),
            Some("1")
        );
        ...
        assert_eq!(response_header(&paused, "x-octet-goal-revision"), Some("2"));
        ...
        assert_eq!(
            response_header(&resumed, "x-octet-goal-revision"),
            Some("3")
        );
        ...
        assert_eq!(
            response_header(&cleared, "x-octet-goal-revision"),
            Some("4")
        );
        ...
        assert_eq!(
            response_header(&after_clear, "x-octet-goal-revision"),
            Some("4")
        );
    ...
    async fn loopback_transport_rejects_cross_origin_and_oversized_requests() {
        ...
        assert_eq!(
            response_header(&allowed, "x-octet-web-bundle"),
    ...
    async fn session_export_is_authenticated_redacted_bounded_and_single_flight() {
        ...
        assert_eq!(
            ...
            Some("attachment; filename=\"octet-session-safe-export.json\"")
        );
        assert_eq!(
        ...
        assert_eq!(body, r#"{"format":"octet-session-export","redacted":true}"#);
