...
pub struct HostCapabilities {
    ...
    pub terminal: bool,
    /// Nested child agents; false until octet implements them.
...
pub struct SessionBranchEntry {
    /// Exact durable octet entry identity.
...
pub enum ItemLifecycle {
    ...
    Provisional,
    /// Committed to octet's append-only session.
    Committed,
}
...
pub enum SourceKind {
    ...
    /// Web URL/result.
    Web,
    /// octet resource such as prompt/skill documentation.
    Resource,
...
pub enum ToolKind {
    ...
    Web,
    /// Read or apply a octet skill/resource.
    Skill,
...
pub struct ExtensionPresentation {
    ...
    pub snapshot: octet_agent::ExtensionPresentationSnapshot,
}
...
mod tests {
    ...
    fn shared_extension_presentation_fixture_is_serve_safe() {
        let snapshot: octet_agent::ExtensionPresentationSnapshot = serde_json::from_str(
            include_str!("../../../crates/octet-coding-agent/fixtures/extension-presentation.json"),
        )
        .unwrap();
