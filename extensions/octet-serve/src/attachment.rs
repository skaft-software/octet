...
impl AttachmentStore {
    /// Opens or creates the store below an already-private octet serve state directory.
...
fn cleanup_temporary_files(directory: &Path) -> Result<(), AttachmentError> {
    ...
    for entry in entries {
        ...
        if name.starts_with(".tmp-")
            || name.starts_with(".octet-tmp-")
            || name.starts_with(".octet-delete-")
        {
...
mod tests {
    ...
    fn startup_cleans_secure_filesystem_recovery_files() {
        ...
        let leftovers = [
            ...
            root.join("metadata").join(".octet-tmp-interrupted-write"),
            root.join("associations")
                .join(".octet-delete-interrupted-cleanup"),
        ];
