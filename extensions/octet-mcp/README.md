...
`octet-mcp` is octet's first-party API `0.2` bridge for explicitly configured
...
tool catalog to octet's transactional `tools/register` and `tools/unregister` API.
...
opt-in from the octet process owner and is not suitable for production or sensitive
...
approval retry only when the host actually negotiates `approvals`; octet `0.7.0`'s
...
Distribution version `0.7.0` retains extension API `0.2`; these numbers are
independent. No public octet bundle availability is asserted.

## Requirements and installation

- octet exactly `0.7.0` (`requires_octet = "=0.7.0"`)
...
Executable extensions run only under octet's documented full-access process gate.
