...
from octet_mcp.config import load_config
...
class ReleaseSmokeTests(unittest.TestCase):
    ...
    def test_release_catalog_and_executable_include_the_self_contained_runtime(self):
        ...
        self.assertIn("octet-mcp", catalog)
        executable = ROOT / "octet-mcp"
    ...
    def test_example_and_real_fixture_configuration_smoke(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            staged_entrypoint = Path(directory) / "octet-mcp"
            staged_entrypoint.write_bytes((ROOT / "octet-mcp").read_bytes())
    ...
    def test_remote_check_config_requires_the_owner_switch(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            config.chmod(0o600)
            staged_entrypoint = root / "octet-mcp"
            staged_entrypoint.write_bytes((ROOT / "octet-mcp").read_bytes())
            staged_entrypoint.chmod(0o755)
            environment = os.environ.copy()
            environment["OCTET_EXTENSION_DIR"] = str(ROOT)
