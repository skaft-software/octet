    ...
    def snapshot(
        ...
        *,
        ...
        snapshot: dict[str, Any] = {
            ...
            "producer": {"id": "octet-mcp", "generation": self.generation},
            "sequence": sequence,
...
def host_presentation(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    """Project the package snapshot onto octet's generic presentation contract."""
...
def compact_status(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    """Return safe semantic status contribution fields for the octet SDK."""
    ...
    suffix = ""
    role = "extension.octet_mcp.ready"
    if bool(summary.get("refreshing")):
        suffix = " · refreshing"
        role = "extension.octet_mcp.refreshing"
    elif bool(summary.get("degraded")):
        suffix = " · degraded"
        role = "extension.octet_mcp.degraded"
    elif configured == 0:
        role = "extension.octet_mcp.empty"
    return {
        "surface": "status",
