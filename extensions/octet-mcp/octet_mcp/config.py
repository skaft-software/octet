...
def default_config_path() -> Path:
    ...
    return Path.home() / ".octet" / "mcp.json"
...
def load_config(
    ...
    *,
    ...
    workspace's ``.octet`` directory and pins its exact SHA-256 digest. Enabled
...
def _read_project_json_file(
    ...
    Resolving ``workspace/.octet`` before the containment check would bless a
    ...
    try:
        ...
        raise ConfigError("a trusted project configuration is outside workspace/.octet") from error
