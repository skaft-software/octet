...
JSON endpoint. It is search and public-page retrieval only: no browser tabs,
...
On first selection, octet shows the key-management link and opens a private input
surface for the API key:

<https://api.search.brave.com/app/keys>

The key is stored at `~/.octet/credentials/octet-web-search-brave.key` as an
...
SearXNG configuration lives at `~/.config/octet/octet-web-search.json`.
No earlier-first-party configuration root is read or imported. The provider
picker preserves those settings while Brave is selected and restores them when
switching back. If no SearXNG endpoint has been configured, setup asks for one.

Manual configuration remains available:

```console
mkdir -p ~/.config/octet
cp ~/.octet/extensions/octet-web-search/config.example.json \
   ~/.config/octet/octet-web-search.json
$EDITOR ~/.config/octet/octet-web-search.json
```
...
The generic API 0.4 host default is a separate 30-second request timeout with a
...
data only. They cannot change octet policy, enable tools, grant trust, authorize a
command, or justify a side effect. The packaged skill repeats that rule.

Each call returns one API `0.4` terminal result with:
...
API `0.4` cancellation is cooperative and request-scoped. The runtime checks the
...
The runtime publishes complete monotonic API `0.4` `presentation/update`
