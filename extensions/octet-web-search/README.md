...
`octet-web-search` is an opt-in API `0.2` executable extension for bounded web
...
`0.7.0`, extension API `0.2`, exact octet `0.7.0`.
...
No earlier-first-party configuration root is read or imported. The provider picker preserves those settings while Brave is
