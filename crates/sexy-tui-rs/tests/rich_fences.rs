...
fn code_block(source: &str) -> sexy_tui_rs::rich_text::CodeBlock {
    ...
    match document.blocks.as_slice() {
        ...
        [Block::Diagram { source, rendered }] => sexy_tui_rs::rich_text::CodeBlock {
...
fn unsupported_bodies_degrade_to_the_original_source() {
    ...
    assert!(!rendered.contains('┌'), "{rendered}");
    assert!(
        rendered.contains("Mermaid diagram not rendered:"),
        "{rendered}"
    );
}
...
fn math_delimiters_dispatch_without_reinterpreting_currency() {
    ...
    assert_eq!(
        markdown::parse("The value is $\\frac{1}{2}$ here.").plain_text(),
        "The value is 1/2 here.\n"
    );
    assert_eq!(
        markdown::parse("Costs $5 and $10 total.").plain_text(),
