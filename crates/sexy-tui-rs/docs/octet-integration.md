# octet integration guide

This is integration guidance, not a library dependency direction. octet keeps all
...
In particular, `crates/octet-coding-agent/src/presentation.rs` should remain the
...
- composer paste classification, attachment chips, slash/path completion,
  submission, focus, and adaptive composer chrome remain in octet;
- terminal-control visualization remains at octet's render boundary. When that
  produces a safe display copy, octet owns a grapheme-safe source/display map,
...
octet computes and passes the usable text-cell width after its prompt marker,
...
keeps cursor coordinates separate from source content; octet inserts its trusted
...
The current octet workspace pins vendored package `0.3.1`. When replacing any
remaining generic mechanisms, preserve these ownership boundaries:

| Current octet location | Replace with |
|---|---|
...
| shell-owned draft string + byte cursor + visual editor layout | `TextEditor`, `TextEditorLayout`, and `TextEditorProjection`; keep attachment and composer policy in octet |
...
octet may retain a thin `OctetTheme` adapter while migrating, but it should delegate
...
octet uses an exact package version and a workspace-relative path:
...
octet's CLI `ColorMode` and explicit plain flag remain application policy. Map them
...
if config.plain {
    ...
    overrides.color_depth = Some(match config.color {
        ...
        // Map Auto/forced modes according to octet's CLI contract.
...
image policy. octet remains responsible for primary-screen raw-mode guards and
panic restoration; the generic TUI also performs Pi-compatible stop cleanup.
Test both cleanup layers as idempotent.

octet's terminal write coalescing recognizes Pi's CSI 2026 frame markers. The
...
octet must leave `TUI::set_inline_scrollback` disabled. The default interactive
...
ledger and changes resize/offscreen-mutation semantics. octet's composer emits a
...
Keep `TranscriptBlock` and its domain variants in octet. Add a conversion at the
view boundary:
...
stable transcript identity/revision (owned by octet)
```
...
octet's raw transcript remains authoritative. Do not replace persisted bytes with
...
Map octet-owned IDs to generic `NodeId`s and retain the returned generation-safe
`NodeHandle`. octet still decides when a status is active, when a tool is committed,
...
Serialize async producer events in octet's event loop and assign monotonically
...
render includes a destination when label and target differ. Keep octet's URL/domain
...
2. **One capability profile:** map CLI flags to overrides and delete duplicate
   detection tests only after equivalent central tests plus octet policy tests pass.
3. **Theme delegation:** keep octet model/background policy, but emit typed colors
...
10. **Delete dead adapters:** remove duplicate dependencies such as `regex` or
    direct `unicode-width` only when no other octet subsystem uses them.

## Acceptance checks in octet
...
These checks preserve the design boundary: octet decides *what happened* and what
