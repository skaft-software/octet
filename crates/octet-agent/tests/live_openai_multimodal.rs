...
//! - `OCTET_LIVE_BASE_URL` (normally ending in `/v1/`)
//! - `OCTET_LIVE_API_KEY`
//! - `OCTET_LIVE_MODEL`
//! - optional `OCTET_LIVE_HEADERS_JSON`, an object of additional headers
...
async fn live_openai_compatible_inline_png_reaches_the_model() {
    let base_url = required("OCTET_LIVE_BASE_URL");
    let api_key = required("OCTET_LIVE_API_KEY");
    let api_name = required("OCTET_LIVE_MODEL");
    let mut headers = http::HeaderMap::new();
    if let Ok(raw) = std::env::var("OCTET_LIVE_HEADERS_JSON") {
        let extra: serde_json::Map<String, serde_json::Value> =
            serde_json::from_str(&raw).expect("OCTET_LIVE_HEADERS_JSON must be a JSON object");
    ...
    // Valid 1x1 PNG. Sending bytes (rather than a URL) verifies octet's inline
    ...
    assert!(matches!(
        ...
        octet_ai::Message::User(user)
            if user.content.iter().any(|part| matches!(part, octet_ai::UserPart::Media(_)))
    ));
}
