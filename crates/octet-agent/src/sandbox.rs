...
//!
//! octet is a trusted local agent, not an OS sandbox. This configuration carries
...
/// intentionally accepted under octet's trusted-local-agent model.
...
mod tests {
    ...
    fn effective_tool_policy_serializes_only_safe_values_and_sources() {
        ...
        let raw_shell_path = PathBuf::from("/private/users/alice/.secrets/octet-shell");
    ...
    fn tilde_expands_to_the_current_users_home_directory() {
        ...
        assert_eq!(
            expand_tilde("~/.octet/config.toml").unwrap(),
