...
pub(super) fn definition(bash: &BashTool) -> ToolDef {
    let mut def = bash.definition();
    def.name = "powershell".into();
    def.description = "Run PowerShell on Windows, preferring pwsh.exe over Windows PowerShell. Uses -NoProfile -NonInteractive -ExecutionPolicy Bypass, bounded output and process-tree cleanup. Never an implicit Bash fallback. Spill output uses full_output_path, or partial_output_path for a prefix when capped or failed; limited to 16 MiB per stream and 64 MiB / 32 files per tool. Paths expire on oldest-first eviction or tool shutdown.".into();
    def
}
...
pub(super) fn resolve_shell() -> Result<std::path::PathBuf, ToolError> {
    ...
    Err(ToolError::new(
        "PowerShell is unavailable: install pwsh.exe or Windows PowerShell",
    ))
}
