...
impl ToolOutput {
    ...
    pub(crate) fn with_owner_presentation_images<'a>(
        ...
        for media in media {
            ...
            count += 1;
            let octet_ai::ImageSource::Inline(bytes) = &image.source else {
                self.presentation_images_omitted = true;
                continue;
            };
            ...
            let owned = Media::Image(octet_ai::ImageMedia {
                source: octet_ai::ImageSource::Inline(bytes::Bytes::copy_from_slice(bytes)),
