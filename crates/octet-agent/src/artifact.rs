...
use bytes::Bytes;
use octet_ai::{AudioFormat, Media, Mime};
...
pub struct ResolvedArtifact {
    ...
    /// Existing octet image/audio value built from the ingested bytes.
    pub media: Media,
}
...
impl ArtifactStore {
    ...
    /// Atomically resolves verified metadata and octet's native media value.
    ...
    /// Resolves only octet's existing image/audio media value.
...
mod tests {
    ...
    use octet_ai::{AudioPayload, ImageSource};
