...
fn parse_base(value: &str) -> Result<url::Url, DeclarationError> {
    ...
    if !matches!(url.scheme(), "http" | "https") || url.host_str().is_none()
        ...
        || (url.query().is_some() && !(query.len() == 1 && query[0].0 == "api-version" && valid_version(&query[0].1))) {
        return Err(invalid());
    }
    let azure_host = url.host_str().is_some_and(|host| [".openai.azure.com", ".cognitiveservices.azure.com", ".ai.azure.com"].iter().any(|suffix| host.ends_with(suffix)));
...
impl AzureRequestOptions {
    ...
    pub fn validate(&self) -> Result<(), DeclarationError> {
        ...
        if self.deployment_name.as_deref().is_some_and(|v| !valid_name(v))
            || self.deployment_map.iter().any(|(key, value)| !valid_name(key) || !valid_name(value))
            || self.resource_name.as_deref().is_some_and(|v| !valid_resource(v))
            || self.api_version.as_deref().is_some_and(|v| !valid_version(v)) {
            return Err(invalid());
        }
        if let Some(base) = &self.base_url { parse_base(base)?; }
        Ok(())
    }
}
...
fn map_from_environment(value: &str) -> Result<BTreeMap<String, String>, AiError> {
    ...
    for entry in value.split(',').map(str::trim).filter(|v| !v.is_empty()) {
        let Some((key, value)) = entry.split_once('=') else { return Err(config_error()); };
        let (key, value) = (key.trim(), value.trim());
        if !valid_name(key) || !valid_name(value) { return Err(config_error()); }
        if map.insert(key.to_owned(), value.to_owned()).is_some_and(|previous| previous != value) {
...
pub(crate) fn apply(
    ...
    if model.endpoint.runtime.responses_profile != ResponsesRuntimeProfile::Azure {
        return if options.is_some() { Err(ConfigError::Parse("Azure overrides require a declared Azure Responses profile".into()).into()) } else { Ok(()) };
    }
    if model.spec.protocol != Protocol::OpenAiResponses { return Err(config_error()); }
    ...
    let (base, explicit_destination) = if let Some(base) = &options.base_url {
        ...
        (format!("https://{resource}.openai.azure.com/openai/v1/"), true)
    } else if let Some(resource) = read("AZURE_OPENAI_RESOURCE_NAME")? {
        if !valid_resource(&resource) { return Err(config_error()); }
        (format!("https://{resource}.openai.azure.com/openai/v1/"), false)
    } else {
    ...
    let configured_version = url.query_pairs().find(|(name, _)| name == "api-version").map(|(_, value)| value.into_owned());
    let version = if let Some(version) = &options.api_version { version.clone() }
        else { read("AZURE_OPENAI_API_VERSION")?.or(configured_version).unwrap_or_else(|| "v1".into()) };
    if !valid_version(&version) { return Err(config_error()); }
    url.set_query(None);
    ...
    let deployment = if let Some(deployment) = &options.deployment_name {
        ...
        read("AZURE_OPENAI_DEPLOYMENT_NAME_MAP")?.map(|value| map_from_environment(&value)).transpose()?
            .and_then(|map| map.get(&model.spec.api_name).cloned()).unwrap_or_else(|| model.spec.api_name.clone())
    };
    if !valid_name(&deployment) { return Err(config_error()); }
...
mod tests {
    ...
    fn azure_base_normalization_is_host_specific_and_rejects_query_credentials() {
        for suffix in ["openai.azure.com","cognitiveservices.azure.com","ai.azure.com"] {
        ...
        for url in ["https://user:secret@resource.openai.azure.com/", "https://resource.openai.azure.com/?secret=value", "https://resource.openai.azure.com/#secret"] {
    ...
    fn explicit_resource_version_and_deployment_mapping_are_resolved_without_network() {
        let mut model = crate::ModelCatalog::builtin().unwrap().resolve(&crate::ModelId("gpt-4o-mini".into())).unwrap();
        ...
        let mut env:BTreeMap<String,String>=["AZURE_OPENAI_BASE_URL","AZURE_OPENAI_RESOURCE_NAME","AZURE_OPENAI_API_VERSION","AZURE_OPENAI_DEPLOYMENT_NAME_MAP"]
            .into_iter().map(|name|(name.into(),String::new())).collect();
        let options=AzureRequestOptions {resource_name:Some("selected-resource".into()),..Default::default()};
        apply(&mut model,Some(&options),&env).unwrap();
        assert_eq!(model.endpoint.base_url.as_str(),"https://selected-resource.openai.azure.com/openai/v1/?api-version=v1");
        assert_eq!(model.spec.api_name,api_name);
        env.insert("AZURE_OPENAI_API_VERSION".into(),"2025-04-01-preview".into());
        env.insert("AZURE_OPENAI_DEPLOYMENT_NAME_MAP".into(),format!("{api_name}=chosen"));
        apply(&mut model,None,&env).unwrap();
        assert_eq!(model.spec.api_name,"chosen");
        assert_eq!(model.endpoint.base_url.query(),Some("api-version=2025-04-01-preview"));
    }
}
