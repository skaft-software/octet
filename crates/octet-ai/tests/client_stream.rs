    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                ...
                .insert_header("content-type", "text/event-stream")
                .insert_header("x-octet-lifecycle", "queued; warming test-api-key"),
        )
        .mount(&mock_server)
        .await;
...
async fn lifecycle_feedback_caps_endpoint_updates() {
    let mock_server = MockServer::start().await;
    let comments = (0..65)
        .map(|index| format!(": octet-lifecycle: loading; update {index}\n\n"))
        .collect::<String>();
    let body = format!(
        "{comments}{}",
...
async fn lifecycle_feedback_is_ignored_without_opt_in() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_string(include_str!("fixtures/openai_chat/lifecycle_feedback.sse"))
                .insert_header("content-type", "text/event-stream")
                .insert_header("x-octet-lifecycle", "queued; ignored test-api-key"),
        )
        .mount(&mock_server)
        .await;
    ...
    assert!(
        !requests[0].headers.contains_key("x-octet-lifecycle"),
...
async fn test_client_stream_non_streaming_chat_audio() {
    ...
    let req = Request {
        ...
        responses: None,
        output_format: OutputFormat::Text,
        output_modalities: OutputModalities::TextAndAudio(octet_ai::AudioOutputOptions {
            format: octet_ai::AudioFormat::Wav,
            voice: octet_ai::AudioVoice::Named("alloy".to_string()),
        }),
        compatibility: Strict,
        cache_retention: octet_ai::CacheRetention::Short,
        session_id: None,
...
async fn responses_credential_resolution_failure_never_opens_a_transport() {
    ...
    impl octet_ai::CredentialResolver for FailedResolver {
...
async fn test_client_stream_http_error_handling() {
    ...
    assert!(res.is_err());
    if let Err(octet_ai::error::AiError::Http(http_err)) = res {
...
async fn strict_pending_media_is_rejected_before_network_io() {
    ...
    assert!(matches!(
        ...
        Err(AiError::Unsupported(octet_ai::UnsupportedError::Image))
    ));
...
async fn completed_response_body_obeys_idle_timeout() {
    ...
    let mut req = text_request();
    req.output_modalities = OutputModalities::TextAndAudio(octet_ai::AudioOutputOptions {
        format: octet_ai::AudioFormat::Wav,
        voice: octet_ai::AudioVoice::Named("alloy".to_string()),
    });
    let client =
        ...
        match event {
            ...
            Err(error)
                if matches!(
                    stream_inner(&error),
                    AiError::Transport(transport)
                        if transport.phase == octet_ai::TransportPhase::Body
                            && transport.timeout
                ) =>
            {
