...
use octet_agent::session::SkillActivatedSnapshot;
...
fn read_bounded_utf8(path: &Path, limit: usize) -> Result<String, PromptError> {
    let bytes =
        octet_agent::secure_fs::read_regular_file_bounded(path, limit).map_err(|error| {
            PromptError::Invalid {
                path: path.to_owned(),
...
pub fn render_and_record(
    ...
    session: &mut octet_agent::Session,
    workspace: &Path,
    name: &str,
    arguments: &str,
...
fn expand_octet_variables(
    ...
    while let Some(start) = rest.find("{{") {
        ...
        let value = octet_variable(variable, raw_prompt, context)?;
        push_bounded(&mut output, &value)?;
        rest = &after[end + 2..];
    }
    push_bounded(&mut output, rest)?;
...
mod tests {
    use super::*;
    use octet_agent::skills::{SkillDescriptor, SkillSource, SkillTrust};
    use octet_agent::EntryId;
    ...
    #[test]
    fn renders_pi_arguments_and_octet_variables_deterministically() {
        let directory = tempfile::tempdir().unwrap();
    ...
    fn prompt_snapshot_does_not_follow_a_file_swapped_to_a_symlink() {
        ...
        let resolver = ResourceResolver::with_global_octet_dir(
    ...
    fn selected_template_name_and_hash_are_durable_session_provenance() {
        ...
        let mut session = octet_agent::Session::create(&session_path).unwrap();
        ...
        let session = octet_agent::Session::open(session_path).unwrap();
        assert!(matches!(
            &session.entries()[0].value,
            octet_agent::EntryValue::PromptTemplateSelected { name, content_hash }
