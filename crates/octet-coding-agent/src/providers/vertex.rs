...
//! values are retained only in this module and in `octet_ai::Auth`.
...
use octet_ai::{Auth, AuthError, CredentialResolver, CredentialScheme, ResolvedCredential, Secret};
...
fn first_environment_value(names: &[&str]) -> anyhow::Result<Option<String>> {
    for name in names {
        let value = match octet_ai::auth::read_bounded_env(name) {
            Ok(value) => value,
