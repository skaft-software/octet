...
fn refresh<F>(path: &std::path::Path, credential: &str, fetch: F) -> anyhow::Result<serde_json::Value>
where
    F: FnOnce(String, http::HeaderMap) -> anyhow::Result<ProviderInventoryResponse>,
{
    refresh_provider_inventory_with(
        path, "fixture", "https://inventory.invalid/models".into(),
        Default::default(), credential, fetch,
    )
}
...
fn seed(path: &std::path::Path) {
    refresh(path, "credential-fingerprint", |_, headers| {
        ...
        Ok(ProviderInventoryResponse::Modified {
            body: json!({"data":[{"id":"last-good"}]}), etag: Some("W/\"opaque-v1\"".into()),
        })
    }).unwrap();
}
...
fn conditional_inventory_http_200_then_304_keeps_body_and_advances_check() {
    ...
    let first = refresh_provider_inventory_with(&path, "fixture", url.clone(), Default::default(), "fingerprint", fetch_provider_inventory).unwrap();
    let mut record = load_provider_inventory_record(&path, "fixture", &url, "fingerprint").unwrap().unwrap();
    assert_eq!(record.etag.as_deref(), Some("W/\"opaque-v1\""));
    record.checked_at = Some(1);
    crate::auth::write_private_atomic(&path, &serde_json::to_vec(&record).unwrap(), ".test-").unwrap();
    let second = refresh_provider_inventory_with(&path, "fixture", url.clone(), Default::default(), "fingerprint", fetch_provider_inventory).unwrap();
    assert_eq!(first, second);
    let record = load_provider_inventory_record(&path, "fixture", &url, "fingerprint").unwrap().unwrap();
...
fn conditional_inventory_does_not_reuse_validators_across_scopes() {
    for (provider, url, fingerprint) in [
        ("different-provider", "https://inventory.invalid/models", "credential-fingerprint"),
        ("fixture", "https://different.invalid/models", "credential-fingerprint"),
        ("fixture", "https://inventory.invalid/models", "different-credential"),
    ] {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("cache.json");
        seed(&path);
        let original = std::fs::read(&path).unwrap();
        let result = refresh_provider_inventory_with(&path, provider, url.into(), Default::default(), fingerprint, |_, headers| {
            assert!(!headers.contains_key(http::header::IF_NONE_MATCH));
...
fn conditional_inventory_200_without_etag_clears_previous_validator() {
    ...
    refresh(&path, "credential-fingerprint", |_, headers| {
        assert!(headers.contains_key(http::header::IF_NONE_MATCH));
        Ok(ProviderInventoryResponse::Modified { body: json!({"data":[]}), etag: None })
    }).unwrap();
    let record = load_provider_inventory_record(&path, "fixture", "https://inventory.invalid/models", "credential-fingerprint").unwrap().unwrap();
...
fn conditional_inventory_rejects_unscoped_304_and_bounds_untrusted_validators() {
    ...
    assert!(!path.exists());
    for invalid in ["".to_owned(), "\"tag\"\r\nAuthorization: stolen".into(), "x".repeat(4097)] {
    ...
    crate::auth::write_private_atomic(&path, &serde_json::to_vec(&legacy).unwrap(), ".test-").unwrap();
    refresh(&path, "credential-fingerprint", |_, headers| {
        assert!(!headers.contains_key(http::header::IF_NONE_MATCH));
        Ok(ProviderInventoryResponse::Modified { body: json!({"data":[]}), etag: Some("\"new\"".into()) })
    }).unwrap();
}
