...
impl CopilotSession {
    ...
    /// a long-lived OAuth token or absolute token-expiry payload to octet.
    pub fn new(
...
/// state themselves. They return only a short-lived [`CopilotSession`] for octet
...
pub trait CopilotHost: Send + Sync {
    ...
    /// refresh token and GitHub OAuth state; octet only receives the replacement
    /// inference session.
...
impl CredentialResolver for CopilotResolver {
    async fn resolve(&self) -> Result<ResolvedCredential, octet_ai::AuthError> {
...
mod tests {
    ...
    async fn endpoint_collisions_leave_discovered_models_out_of_the_catalog() {
        ...
        catalog
            .register_endpoint(octet_ai::Endpoint {
                id: octet_ai::EndpointId("github-copilot-chat".to_owned()),
        ...
        assert_eq!(catalog.models().count(), 0);
        assert!(!catalog.has_endpoint(&octet_ai::EndpointId("github-copilot-responses".to_owned())));
    }
    ...
    async fn authenticated_model_metadata_selects_each_declared_protocol_route() {
        ...
        let chat = catalog
            .resolve(&octet_ai::ModelId("github-copilot/chat-model".to_owned()))
            .unwrap();
