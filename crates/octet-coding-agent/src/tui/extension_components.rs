...
impl std::fmt::Display for ComponentSurfaceError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::BoundsExceeded(detail) => write!(f, "component surface bounds exceeded: {detail}"),
...
impl ExtensionComponentSurface {
    ...
    pub fn invalidate(&mut self, id: &str) -> Result<(), ComponentSurfaceError> {
        ...
        if !self.components.contains_key(id) {
            ...
            self.components.insert(id.to_owned(), ComponentRegion::default());
        }
    ...
    pub fn store_render(
        ...
        bounded_id(id)?;
        bounded_lines(&lines, MAX_RENDER_LINES, MAX_RENDER_LINE_BYTES, "component render")?;
    ...
    pub fn widget_set(
        ...
        match content {
            ...
            Some(WidgetContent::Lines(lines)) => {
                ...
                self.widgets
                    .insert(key.to_owned(), (WidgetContent::Lines(lines), placement.unwrap_or_default()));
                Ok(())
            }
    ...
    pub fn widgets(
        &self,
    ) -> impl Iterator<Item = (&String, &WidgetContent, WidgetPlacement)> + '_ {
        self.widgets
            .iter()
...
pub fn raw_key_data(code: crossterm::event::KeyCode, modifiers: crossterm::event::KeyModifiers) -> String {
...
mod tests {
    ...
    fn live_component_bound_is_enforced() {
        ...
        for index in 0..MAX_LIVE_COMPONENTS {
            surface
                .invalidate(&format!("component.{index}"))
                .unwrap();
        }
        assert_eq!(
    ...
    fn widget_set_accepts_lines_and_component_forms_and_clears() {
        ...
        surface
            ...
            .unwrap();
        surface
            .widget_set("clock", Some(WidgetContent::Component("clock.1".into())), None)
            .unwrap();
        ...
        assert!(matches!(
            surface.widget_set(
                "big",
                Some(WidgetContent::Lines(vec![long_line])),
                None
            ),
            Err(ComponentSurfaceError::BoundsExceeded(_))
        ));
    ...
    fn header_footer_editor_slots_route_to_component_regions() {
        let mut surface = surface();
        surface.slot_set(ComponentSlot::Header, Some("head.1")).unwrap();
        surface.slot_set(ComponentSlot::Footer, Some("foot.1")).unwrap();
        surface.slot_set(ComponentSlot::Editor, Some("edit.1")).unwrap();
    ...
    fn raw_key_data_matches_terminal_spellings() {
        ...
        assert_eq!(
            raw_key_data(KeyCode::Char('a'), KeyModifiers::ALT),
            "\x1ba"
        );
        assert_eq!(raw_key_data(KeyCode::F(5), KeyModifiers::NONE), "\x1b[5~");
        assert_eq!(raw_key_data(KeyCode::PageDown, KeyModifiers::NONE), "\x1b[6~");
    }
