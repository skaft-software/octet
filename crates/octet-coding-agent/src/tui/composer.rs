...
mod tests {
    ...
    fn compose_splits_text_and_media_preserving_order() {
        ...
        assert!(
            matches!(&composed.parts[0], octet_agent::InputPart::Text(t) if t.trim() == "before")
        );
        assert!(matches!(
            &composed.parts[1],
            octet_agent::InputPart::Media(octet_ai::Media::Image(_))
        ));
        assert!(
            matches!(&composed.parts[2], octet_agent::InputPart::Text(t) if t.trim() == "after")
        );
        assert!(ledger.is_empty());
    }
    ...
    fn compose_drops_orphans_and_keeps_mangled_chips_literal() {
        ...
        assert_eq!(composed.parts.len(), 1);
        assert!(
            matches!(&composed.parts[0], octet_agent::InputPart::Text(t) if t == "[Image #1: shot.pn")
        );
        assert!(ledger.is_empty());
    }
