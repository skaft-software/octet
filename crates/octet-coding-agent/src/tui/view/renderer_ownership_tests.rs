...
fn isolated_structural_rebuild_cannot_reuse_the_retained_frame_generation() {
    for insert in [false, true] {
        ...
        assert_eq!(
            ...
            "a new root cannot certify old rows"
        );
    ...
    let observer = std::thread::spawn(move || {
        ...
        loop {
            ...
            assert!(
                ...
                "latest publication was not painted"
            );
            std::thread::sleep(Duration::from_millis(1));
        }
...
fn rapid_page_and_wheel_navigation_reuse_painted_history_after_draft_edits() {
    for page in [false, true] {
        ...
        let maximum = {
            ...
            super::super::max_scroll_for_available(geometry.total_rows, geometry.viewport_available)
        };
        assert!(maximum > 50);
