...
pub(super) fn document_content_width(theme: &OctetTheme, width: u16) -> u16 {
...
fn panel_filter_line(theme: &OctetTheme, filter: &str, width: u16) -> String {
    let terminal_width = width;
    let plan = PresentationLayout::new(theme, width);
    let width = usize::from(plan.content_width);
...
mod grouped_model_tests {
    ...
    fn action() -> PanelAction {
        PanelAction::SelectGroupedModel {
            models: vec![
                octet_ai::ModelId("a".into()),
                octet_ai::ModelId("b".into()),
