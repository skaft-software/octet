...
impl AssistantBlock {
    ...
    pub(super) fn materialize_source(&mut self, source: &SharedText, previous: Option<&mut Self>, reasoning: bool) {
        let finished = self.finished;
        let mut rendered = previous.map(|old| std::mem::replace(old, Self::streaming("")))
            .unwrap_or_else(|| if reasoning { Self::streaming_reasoning("") } else { Self::streaming("") });
