...
/// frame-boundary callback exposed by its `Terminal` trait.  octet's backend
...
fn sexy_terminal_capabilities(
    ...
    // This is both a progressive terminal feature and octet's render-frame
...
where
    S: futures_util::Stream<Item = std::io::Result<event::Event>> + Unpin,
{
...
mod tests {
    ...
    fn tui_write_log_captures_the_exact_normalized_backend_bytes_when_enabled() {
        ...
        let mut terminal = OctetTerminal {
            out: writer,
