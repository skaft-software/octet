...
impl ContextReport {
    ...
    #[cfg(test)]
    pub(crate) fn render(&self, theme: &OctetTheme, width: u16) -> Vec<String> {
    ...
    pub(crate) fn render_body(&self, theme: &OctetTheme, width: u16) -> Vec<String> {
        let width = width.max(1);
    ...
    fn render_details(&self, theme: &OctetTheme, width: u16) -> Vec<String> {
    ...
    fn render_grid(&self, theme: &OctetTheme, width: u16, spaced: bool) -> Vec<String> {
        let (columns, _) = grid_dimensions(self.context_window);
        self.grid_cells()
            .chunks(columns)
            .map(|row| {
...
fn render_grid_cell(cell: GridCell, theme: &OctetTheme) -> String {
...
fn render_slice(slice: &ContextSlice, theme: &OctetTheme, width: u16, window: u64) -> String {
