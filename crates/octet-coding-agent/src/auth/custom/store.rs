...
//! File-backed custom endpoint store at `~/.octet/credentials/custom.json`.
...
use octet_ai::CacheCompatibility;
...
pub struct CustomCredential {
    ...
    /// When true (the default), octet calls GET /v1/models on the endpoint at
...
pub struct CustomModel {
    ...
    pub api_name: String,
    /// Optional display name in octet's model picker. Defaults to api_name.
    #[serde(default)]
    ...
    /// Optional user-declared pricing. When absent, octet treats the model as
...
/// (including the zero default octet applies) enables cost guardrails such as
...
/// Default store path: `~/.octet/credentials/custom.json`.
...
impl CredentialStore {
    ...
    pub(crate) fn save_registry_if_unchanged(
        ...
        match octet_agent::secure_fs::write_private_atomic_if_unchanged(
            ...
            Ok(()) => Ok(()),
            Err(octet_agent::secure_fs::SecureFileError::Changed) => {
...
mod tests {
    ...
    fn write_private_fixture(path: &Path, bytes: impl AsRef<[u8]>) {
        octet_agent::secure_fs::write_private_atomic(path, bytes.as_ref(), 64 * 1024).unwrap();
    }
