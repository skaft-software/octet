...
pub(crate) fn subscription_claims(access: &str) -> Result<SubscriptionClaims> {
    ...
    {
        bail!(
            "stored OpenAI credential is localhost-only and cannot access the ChatGPT model pool; run `octet --login codex` again"
        );
    }
    let account_id = auth
...
/// session rather than the localhost-only token produced by the old octet
