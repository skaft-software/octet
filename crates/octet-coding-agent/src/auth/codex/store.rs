...
//! File-backed credential store at `~/.octet/credentials/codex.json` (mode 0600).
...
/// Default store path: `~/.octet/credentials/codex.json`.
...
impl CredentialStore {
    ...
    fn open_model_cache(&self) -> Result<Option<std::fs::File>> {
        ...
        match octet_agent::secure_fs::open_private_file_for_read(&path) {
    ...
    /// Codex CLI store contains one. A default store imports at most once while holding
    /// the same cross-process lock used for token rotation.
    pub fn load(&self) -> Result<Option<CredentialFile>> {
        if let Some(credential) = self.load_octet_credential()? {
    ...
    fn load_octet_credential(&self) -> Result<Option<CredentialFile>> {
    ...
    /// Read only the octet-owned credential while the caller owns the refresh
    ...
    /// serialized with refresh-token rotation across octet processes.
...
fn write_private(path: &Path, bytes: &[u8]) -> std::io::Result<()> {
    octet_agent::secure_fs::write_private_atomic(path, bytes, MAX_MODEL_CACHE_BYTES)
...
mod tests {
    ...
    fn refresh_lock_waits_on_the_private_credential_directory() {
        ...
        let blocker =
            octet_agent::secure_fs::open_private_directory_for_lock(&lock_directory).unwrap();
    ...
    fn refresh_lock_remains_serialized_when_credential_file_is_replaced() {
        ...
        octet_agent::secure_fs::write_private_atomic(&path, br#"{"rotated":true}"#, 1024).unwrap();
    ...
    fn imports_codex_cli_credentials_once_without_modifying_the_source() {
        ...
        let path = dir.path().join("octet/credentials/codex.json");
    ...
    #[test]
    fn earlier_first_party_credentials_are_never_imported_or_modified() {
        let dir = tempfile::tempdir().unwrap();
        let home = dir.path().join("home");
        let hamr_path = home.join(".hamr/agent/auth.json");
        let ygg_path = home.join(".ygg/credentials/codex.json");
        let hamr = br#"{"openai-codex":{"access":"access","refresh":"refresh","expires":4102444800,"accountId":"acct"}}"#;
        let ygg = br#"{"tokens":{"access_token":"access","refresh_token":"refresh","account_id":"acct"},"expires_at":4102444800}"#;
        for (path, bytes) in [(&hamr_path, hamr.as_slice()), (&ygg_path, ygg.as_slice())] {
            std::fs::create_dir_all(path.parent().unwrap()).unwrap();
            std::fs::write(path, bytes).unwrap();
        }
        let path = home.join(".octet/credentials/codex.json");
        let store = CredentialStore::with_legacy_home(&path, &home);
        assert!(store.load().unwrap().is_none());
        // An unusable third-party credential is not permission to try an old
        // first-party root either. All inputs here are synthetic temporary files.
        let codex_path = home.join(".codex/auth.json");
        std::fs::create_dir_all(codex_path.parent().unwrap()).unwrap();
        std::fs::write(&codex_path, b"not JSON").unwrap();
        assert!(store.load().unwrap().is_none());
        assert!(!path.exists());
        assert!(!store.migration_marker_path().exists());
        assert_eq!(std::fs::read(&hamr_path).unwrap(), hamr);
        assert_eq!(std::fs::read(&ygg_path).unwrap(), ygg);
        assert_eq!(std::fs::read(&codex_path).unwrap(), b"not JSON");
    }
    ...
    fn logout_rejects_a_symlinked_marker_without_touching_private_state() {
        ...
        assert!(std::fs::symlink_metadata(store.migration_marker_path())
            ...
            .file_type()
            .is_symlink());
        assert!(store.load_octet_credential().unwrap().is_some());
    }
