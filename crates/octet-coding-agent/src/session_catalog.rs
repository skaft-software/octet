...
mod tests {
    ...
    #[test]
    fn large_uncheckpointed_catalog_preserves_rows_on_reopen() {
        let temp = tempfile::tempdir().unwrap();
        let mut catalog = SessionCatalog::open(temp.path()).unwrap();
        catalog
            .apply_entries(
                &[update("retained", &["still discoverable"])],
                &HashSet::new(),
            )
            .unwrap();
        catalog
            ...
            .unwrap();
        catalog.connection.execute_batch("CREATE TABLE legacy_padding (value BLOB); INSERT INTO legacy_padding VALUES (zeroblob(65 * 1024 * 1024));").unwrap();
        let reopened = SessionCatalog::open_recovering(temp.path()).unwrap();
        assert_eq!(
            reopened.search_entries("discoverable", 10).unwrap()[0].session_id,
            "retained"
        );
    }
