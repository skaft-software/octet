...
pub fn context_window(model: &octet_ai::Model) -> u64 {
...
pub(crate) mod tests {
    ...
    async fn manual_compaction_refuses_invalid_main_or_split_summary_without_discarding_history() {
        for (responses, expected_calls) in [
            ...
            assert_eq!(
                format!("{:?}", app.agent.session().context().unwrap()),
                before
            );
            assert!(app
                .agent
    ...
    async fn local_compaction_preserves_encrypted_tail_across_responses_resume() {
        ...
        let agent_for_session = |session| {
            octet_agent::Agent::new(octet_agent::AgentConfig {
                client: client.clone(),
                model: model.clone(),
                session,
                system: "system".into(),
                sandbox: octet_agent::SandboxConfig::new(directory.path()),
                effect_broker: octet_agent::EffectBroker::new(
        ...
        let encode = |session: &Session| {
            ...
            octet_ai::responses::encode_responses_replay(&model, Some("system"), &replay).unwrap()
        };
