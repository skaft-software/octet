...
fn media_marker(media: &octet_ai::Media) -> String {
    match media {
        octet_ai::Media::Image(image) => {
            let mime = image
                .media_type
                .as_ref()
            ...
            match &image.source {
                octet_ai::ImageSource::Inline(data) => {
                    format!("[image {mime} · {}]", human_bytes(data.len() as u64))
                }
                _ => format!("[image {mime}]"),
            }
        }
        octet_ai::Media::Audio(audio) => {
            let format = format!("{:?}", audio.format).to_lowercase();
            match &audio.payload {
                octet_ai::AudioPayload::Inline(data) => {
...
fn active_branch_tail_from<'a>(
    session: &'a Session,
    head: Option<&'a octet_agent::EntryId>,
...
mod tests {
    use super::*;
    use octet_agent::{EntryMetadata, EntryValue, Session};
    use octet_ai::{
    ...
    fn resumed_native_compaction_renders_only_a_payload_free_marker() {
        ...
        session
            .append(EntryValue::ResponsesCompaction {
                endpoint: octet_ai::EndpointId("responses".into()),
                model: ModelId("gpt-test".into()),
                covered_through,
                output: octet_ai::ResponsesOutput::new(vec![octet_ai::ResponsesItem::new(
                    serde_json::json!({
                        "type": "compaction",
    ...
    fn responses_sidecar_is_transparent_to_tool_pair_tail_hydration() {
        ...
        session
            .append_responses_turn(
                assistant,
                octet_ai::EndpointId("responses-endpoint".into()),
                ModelId("responses-model".into()),
                octet_ai::ResponsesOutput::new(vec![octet_ai::ResponsesItem::new(
                    serde_json::json!({
                        "type": "function_call",
                        "id": "fc_responses",
