...
use octet_agent::{public_error_diagnostic, AgentEvent, FinishReason};
...
impl HostRunOutcome {
    ...
    pub fn extension_lifecycle_outcome(&self) -> octet_agent::ExtensionLifecycleOutcome {
        match self {
            Self::Completed => octet_agent::ExtensionLifecycleOutcome::Completed,
            Self::Aborted => octet_agent::ExtensionLifecycleOutcome::Cancelled,
            Self::MaxTurns => octet_agent::ExtensionLifecycleOutcome::LimitReached,
            Self::Failed(_) => octet_agent::ExtensionLifecycleOutcome::Failed,
            Self::StreamLost => octet_agent::ExtensionLifecycleOutcome::FrontendDisconnected,
            Self::Shutdown => octet_agent::ExtensionLifecycleOutcome::Shutdown,
        }
    }
}
...
mod tests {
    ...
    fn run_finished_reasons_map_to_one_shared_host_outcome() {
        ...
        let failed = HostRunOutcome::from_event(
            &finished(FinishReason::Failed(octet_agent::AgentError::RunEnded)),
            "provider",
            "model",
        )
    ...
    fn every_host_terminal_outcome_has_an_api_0_2_lifecycle_outcome() {
        use octet_agent::ExtensionLifecycleOutcome as Lifecycle;
    ...
    fn nonterminal_events_do_not_settle_the_host_boundary() {
        let event = AgentEvent::OutputDelta {
            channel: octet_agent::OutputChannel::Text,
            text: "partial".into(),
        };
        assert_eq!(
