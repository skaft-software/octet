...
use octet_agent::{AgentEvent, FinishReason, ToolError, ToolOutput, ToolOutputMediaKind};
use octet_ai::{
...
mod tests {
    ...
    use octet_agent::{AgentError, EntryId, FinishReason, OutputChannel};
    use octet_ai::{AiError, AssistantMessage, ModelId, Protocol, ToolCall, ToolCallId, Usage};
    ...
    fn explicit_registry_and_custom_display_names_follow_precedence() {
        ...
        let registry = octet_ai::model_metadata::model_display_name("openai/gpt-4o-mini").unwrap();
        assert_eq!(
    ...
    fn tool_summaries_show_real_commands() {
        let read = summarize_tool(
            "read",
            &serde_json::json!({"path":"crates/octet/src/lib.rs"}),
        );
        assert_eq!(read.active, "reading crates/octet/src/lib.rs");
