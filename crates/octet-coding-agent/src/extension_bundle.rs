...
pub(super) async fn install_official(
    ...
    if !is_official_bundle(id) {
        anyhow::bail!(
            "unknown official extension bundle {id:?}; use 'octet extension install --path ARCHIVE' for a local bundle"
        );
    }

    let octet_version = Version::parse(env!("CARGO_PKG_VERSION"))?;
    let asset = format!("{id}-{octet_version}.tar.gz");
    let tag = format!("v{octet_version}");
    ...
    let temporary = tempfile::Builder::new()
        .prefix("octet-extension-download-")
        .tempdir()
...
fn install_archive(
    ...
    let staging = tempfile::Builder::new()
        .prefix(".octet-extension-install-")
        .tempdir_in(root)
    ...
    let manifest = octet_agent::ExtensionManifest::load_bounded(&manifest_path, MAX_MANIFEST_BYTES)
        .with_context(|| {
            format!(
    ...
    match fs::symlink_metadata(&destination) {
        Ok(metadata) => {
            ...
            if !replace {
                anyhow::bail!(
                    "{} is already installed; run 'octet extension update {}' for an official bundle",
                    installed.id,
                    installed.id
                );
            }
        ...
        Err(error) if error.kind() == io::ErrorKind::NotFound => {
            if replace {
                anyhow::bail!(
                    "{} is not installed; run 'octet extension install {}'",
                    installed.id,
                    installed.id
                );
            }
...
fn validate_bundle_manifest(
    manifest: &octet_agent::ExtensionManifest,
    archive_root: &str,
) -> anyhow::Result<InstalledBundleManifest> {
    ...
    let requires_octet = manifest.requires_octet.as_deref().ok_or_else(|| {
        anyhow::anyhow!(
            "installable extension manifest must declare requires_octet = {expected_requirement:?}"
        )
    })?;
    let requirement = VersionReq::parse(requires_octet)
        .context("extension requires_octet is not a valid semantic version requirement")?;
    if requires_octet != expected_requirement || !requirement.matches(&current) {
        anyhow::bail!(
            "extension requires octet {requires_octet:?}; this release requires exact compatibility {expected_requirement:?}"
        );
    }
    ...
    Ok(InstalledBundleManifest {
        ...
        requires_octet: requires_octet.to_owned(),
    })
}
...
fn write_install_record(
    ...
    let record = InstallRecord {
        ...
        version: manifest.version.clone(),
        api_version: manifest.api_version.clone(),
        requires_octet: manifest.requires_octet.clone(),
        source_kind,
        source: source.to_owned(),
        archive_sha256: archive_sha256.to_owned(),
        installed_by_octet: env!("CARGO_PKG_VERSION").to_owned(),
    };
...
fn validate_install_record(record: &InstallRecord, expected_id: &str) -> anyhow::Result<()> {
    ...
    if !octet_agent::extension_api_v03::bundle_supports_api_version(&record.api_version) {
        anyhow::bail!(
            "installed extension has unsupported bundle API {:?}",
            record.api_version
        );
    }
    let installed_by_octet = Version::parse(&record.installed_by_octet)
        .context("installed_by_octet is not semantic versioning")?;
    let requirement = VersionReq::parse(&record.requires_octet)
        .context("installed extension requires_octet is invalid")?;
    let expected_requirement = format!("={installed_by_octet}");
    if record.requires_octet != expected_requirement || !requirement.matches(&installed_by_octet) {
        anyhow::bail!("installed extension octet requirement is not the exact installer version");
    }
...
pub(super) fn list_installed(root: &Path) -> anyhow::Result<Vec<InstalledBundle>> {
    ...
    for entry in entries {
        ...
        installed.push(InstalledBundle {
            ...
            requires_octet: record.requires_octet,
...
pub(super) fn installed_skill_roots(root: &Path) -> Vec<PathBuf> {
    ...
    let mut roots = entries
        ...
        .filter_map(|entry| {
            ...
            let compatible = VersionReq::parse(&record.requires_octet)
                .ok()?
                .matches(&current);
            if !compatible {
                return None;
...
mod tests {
    ...
    fn create_bundle(directory: &Path, id: &str, body: &[u8]) -> PathBuf {
        create_bundle_with_api_version(directory, id, body, octet_agent::EXTENSION_API_VERSION)
    }
    ...
    fn local_bundle_installs_lists_updates_atomically_and_removes() {
        ...
        assert_eq!(record.api_version, octet_agent::EXTENSION_API_VERSION);
        assert_eq!(
            record.requires_octet,
        ...
        assert_eq!(listed[0].api_version, octet_agent::EXTENSION_API_VERSION);
    ...
    fn bundle_version_policy_keeps_api_v02_installable() {
        ...
        for version in ["0.2", "0.3"] {
            ...
            let parsed = octet_agent::ExtensionManifest::parse(&manifest).unwrap();
            assert_eq!(
        ...
        let parsed = octet_agent::ExtensionManifest::parse(&legacy).unwrap();
