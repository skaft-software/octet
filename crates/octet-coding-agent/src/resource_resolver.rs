...
/// A filesystem resource family understood by octet.
...
pub struct ResourceResolver {
    workspace: PathBuf,
    global_octet_dir: Option<PathBuf>,
    workspace_trusted: bool,
}

impl ResourceResolver {
    pub fn new(workspace: PathBuf, workspace_trusted: bool) -> Self {
        if let Some(global_octet_dir) = global_octet_dir_from_home(dirs::home_dir()) {
            return Self::with_global_octet_dir(workspace, workspace_trusted, global_octet_dir);
        }
        Self {
            workspace,
            global_octet_dir: None,
            workspace_trusted,
        }
    }
    ...
    pub fn with_global_octet_dir(
        workspace: PathBuf,
        workspace_trusted: bool,
        global_octet_dir: PathBuf,
    ) -> Self {
        Self {
            workspace,
            global_octet_dir: Some(global_octet_dir),
            workspace_trusted,
        }
    }
    ...
    pub fn discover(&self, kind: ResourceKind, explicit_roots: &[PathBuf]) -> ResourceSnapshot {
        ...
        if let Some(global_octet_dir) = &self.global_octet_dir {
            self.scan_root(
                kind,
                &global_octet_dir.join(kind.directory_name()),
            ...
            diagnostics.push(ResourceDiagnostic {
                ...
                kind,
                path: PathBuf::from("~/.octet").join(kind.directory_name()),
                message: "global resources disabled because the user home directory is unavailable"
                    .into(),
            });
        }

        let project_root = self.workspace.join(".octet").join(kind.directory_name());
...
fn global_octet_dir_from_home(home: Option<PathBuf>) -> Option<PathBuf> {
    home.filter(|home| home.is_absolute())
        .map(|home| home.join(".octet"))
}
...
mod tests {
    ...
    fn resolver(root: &tempfile::TempDir, trusted: bool) -> (ResourceResolver, PathBuf, PathBuf) {
        ...
        let global = root.path().join("global/.octet");
        std::fs::create_dir_all(&workspace).unwrap();
        (
            ResourceResolver::with_global_octet_dir(workspace.clone(), trusted, global.clone()),
            workspace,
            global,
        )
    }
    ...
    fn unavailable_home_never_reclassifies_project_files_as_global() {
        ...
        let workspace = root.path().join("workspace");
        let project = workspace.join(".octet/extensions/local-tool");
    ...
    fn relative_home_is_not_a_global_resource_root() {
        assert_eq!(global_octet_dir_from_home(None), None);
        assert_eq!(global_octet_dir_from_home(Some(".".into())), None);
        let absolute_home = std::env::temp_dir().join("octet-home");
        assert_eq!(
            global_octet_dir_from_home(Some(absolute_home.clone())),
            Some(absolute_home.join(".octet"))
        );
    }
    ...
    fn untrusted_project_is_absent_and_diagnosed() {
        ...
        std::fs::create_dir_all(workspace.join(".octet/prompts")).unwrap();
        std::fs::write(global.join("prompts/review.md"), "global").unwrap();
        std::fs::write(workspace.join(".octet/prompts/review.md"), "project").unwrap();
