...
use octet_agent::{EXTENSION_API_VERSION_0_2, EXTENSION_API_VERSION_0_3};
...
const PI_PLAN_SCHEMA: &str = "octet.pi.aggregate-plan.v1";
const PI_RUNTIME_EVIDENCE_SCHEMA: &str = "octet.pi.runtime.evidence.v1";
...
pub enum PiCommand {
    ...
    Install {
        ...
        /// Override the generated octet extension name.
        #[arg(long)]
        name: Option<String>,
        ...
        /// octet extension root used for the generated compatibility link.
        #[arg(long, value_name = "DIR")]
    ...
    Preflight {
        /// Canonical plan generated by `octet pi plan`.
        plan: PathBuf,
    },
    ...
    Rollback {
        ...
        name: String,
        /// octet extension root used for generated compatibility links.
        #[arg(long, value_name = "DIR")]
        extension_root: Option<PathBuf>,
    },
...
impl PiAggregatePlan {
    fn new(
        ...
        let mut plan = Self {
            ...
            octet_version: OCTET_VERSION.to_owned(),
        ...
        PiCommand::Plan {
            ...
            if let Some(output) = output {
                ...
                crate::output::stdout_line(
                    "Pi aggregate plan written. Run `octet pi preflight --plan FILE` before publishing.",
                );
                crate::output::stdout_line(
...
fn read_plan(requested_plan: &Path, invocation_cwd: &Path) -> anyhow::Result<PiAggregatePlan> {
    ...
    let bytes = octet_agent::secure_fs::read_regular_file_bounded(&plan, MAX_PI_PLAN_BYTES)
...
fn preflight_plan(plan: &PiAggregatePlan) -> anyhow::Result<()> {
    ...
    if actual_digest != plan.plan_digest {
        anyhow::bail!(
            "Pi aggregate plan changed after it was compiled; review sources and run `octet pi plan` again"
        );
    }
...
fn publish_plan_for_api(
    ...
    fs::create_dir_all(&extension_root).with_context(|| {
        format!(
            "cannot create octet extension root {}",
            extension_root.display()
        )
    })?;
    reject_symlink(&extension_root, "octet extension root")?;
    let extension_root = extension_root.canonicalize().with_context(|| {
        format!(
            "cannot resolve octet extension root {}",
            extension_root.display()
        )
    })?;
    ...
    match fs::symlink_metadata(&package) {
        Ok(_) => anyhow::bail!(
            "Pi compatibility link {:?} already exists; run `octet pi rollback {}` or choose a new name",
            plan.name,
            plan.name
        ),
    ...
    let publish_result = (|| -> anyhow::Result<()> {
        publication.write_private_file(
            &bridge_path,
            include_str!("../../../extensions/octet-pi-compat/bridge.mjs"),
        )?;
        #[cfg(unix)]
        {
        ...
        // so another octet process cannot launch a partially generated package.
...
fn rollback(
    ...
    reject_symlink(&root, "octet extension root")?;
...
fn load_installation(package: &Path) -> Option<ParsedPiInstallation> {
    ...
    if let Ok(bytes) =
        octet_agent::secure_fs::read_regular_file_bounded(&lock_path, MAX_PI_LOCK_BYTES)
    {
    ...
    let bytes =
        octet_agent::secure_fs::read_regular_file_bounded(&record_path, MAX_LINK_RECORD_BYTES)
            .ok()?;
...
fn manifest_for_plan(
    ...
    // octet stages the selected entrypoint bytes before execution. On Unix,
    ...
    // resolve Node from octet's sanitized PATH instead.
    ...
    entrypoint_args.extend([
        ...
        "--octet-version".to_owned(),
        plan.octet_version.clone(),
    ...
    Ok(ExtensionManifest {
        ...
        requires_octet: Some(format!("={OCTET_VERSION}")),
...
fn validate_plan_shape(plan: &PiAggregatePlan) -> anyhow::Result<()> {
    ...
    if plan.bridge_version != BRIDGE_VERSION
        ...
        || plan.octet_version != OCTET_VERSION
...
fn aggregate_plan_digest(plan: &PiAggregatePlan) -> anyhow::Result<String> {
    ...
    hasher.update(b"octet-pi-aggregate-plan\0");
    hasher.update(bytes);
...
fn link_record_digest(record: &PiLinkRecord) -> anyhow::Result<String> {
    ...
    let bytes = serde_json::to_vec(&unsigned)?;
    let mut hasher = Sha256::new();
    hasher.update(b"octet-pi-link-record\0");
    hasher.update(bytes);
    Ok(format!("{:x}", hasher.finalize()))
}
...
fn aggregate_lock_digest(record: &PiLockRecord) -> anyhow::Result<String> {
    ...
    let bytes = serde_json::to_vec(&unsigned)?;
    let mut hasher = Sha256::new();
    hasher.update(b"octet-pi-aggregate-lock\0");
    hasher.update(bytes);
    Ok(format!("{:x}", hasher.finalize()))
}
...
fn link_identity(
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"octet-pi-aggregate-link-identity\0");
...
fn runtime_evidence_text(
    ...
    let evidence = serde_json::json!({
        ...
        "schema_version": 1,
        "api": {
            "version": octet_agent::extension_api_v03::API_VERSION,
            "schema": octet_agent::extension_api_v03::SCHEMA_ID,
            "schema_sha256": octet_agent::extension_api_v03::SCHEMA_SHA256,
        },
    ...
    let canonical = octet_agent::extension_api_v03::canonical_json(&evidence)
...
fn link_status(record: &ParsedPiLinkRecord) -> String {
    ...
    if record.octet_version != OCTET_VERSION {
        stale.push("octet version changed".to_owned());
    }
...
fn fingerprint_source_with_limits(
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"octet-pi-source-fingerprint\0");
...
fn hash_regular_file(
    ...
    let mut file = octet_agent::secure_fs::open_regular_file_for_read(path)
...
fn fingerprint_source_locks(source: &Path) -> anyhow::Result<SourceLockFingerprint> {
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"octet-pi-source-lock-fingerprint\0");
...
fn pi_runtime_integrity(root: &Path) -> anyhow::Result<PiRuntimeIntegrity> {
    let manifest = octet_agent::secure_fs::read_regular_file_bounded(
        &root.join("package.json"),
        MAX_PI_PACKAGE_MANIFEST_BYTES,
    )?;
    let distribution = fingerprint_source(&root.join("dist"))?;
    let mut hasher = Sha256::new();
    hasher.update(b"octet-pi-runtime-integrity\0");
...
fn resolve_pi_runtime(
    ...
    for variable in ["OCTET_PI_PACKAGE", "PI_CODING_AGENT_PACKAGE"] {
...
fn resolve_extension_root(requested: Option<&Path>, cwd: &Path) -> anyhow::Result<PathBuf> {
    ...
    let home = dirs::home_dir().ok_or_else(|| anyhow::anyhow!("user home is unavailable"))?;
    absolute_path(&home.join(".octet/extensions"), cwd)
}
...
fn write_new_private_file(path: &Path, text: &str) -> anyhow::Result<()> {
    ...
    .map_err(|_| {
        anyhow::anyhow!("cannot safely create Pi aggregate plan output; choose a private new file")
    })
}
...
impl PackagePublication {
    fn create(path: &Path) -> anyhow::Result<Self> {
        ...
        if let Err(error) = octet_agent::secure_fs::create_private_directory_all(path) {
    ...
    fn rollback(&mut self) -> anyhow::Result<()> {
        ...
        for path in self.files.iter().rev() {
            if let Err(error) = octet_agent::secure_fs::remove_regular_file_if_exists(path) {
...
mod tests {
    ...
    fn v3_record_manifest_and_evidence_bind_exact_compatibility_identity() {
        ...
        assert_eq!(value["octet_version"], OCTET_VERSION);
        assert_eq!(
        ...
        assert_eq!(
            ...
            octet_agent::extension_api_v03::API_VERSION
        );
        assert_eq!(
            octet_agent::extension_api_v03::canonical_json(&evidence_value).unwrap(),
            evidence.trim()
        );
    }
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn generated_link_negotiates_runtime_commands_with_the_real_octet_host() {
        ...
        let catalog = octet_agent::ExtensionCatalog::load_resolved(
            [octet_agent::extension_process::ExtensionManifestInput {
                path: manifest_path,
                source: octet_agent::ExtensionSource::Explicit,
            }],
            &policy,
            64 * 1024,
        );
        ...
        let process = octet_agent::ExtensionProcess::start(
            descriptor,
            octet_agent::ExtensionRuntimeConfig::new(temp.path()),
        )
        .await
        .unwrap();
        assert!(process
            .negotiated_features()
            .contains(octet_agent::EXTENSION_FEATURE_RUNTIME_COMMANDS));
        ...
        .unwrap();
        let stale_error = match octet_agent::ExtensionProcess::start(
            stale_descriptor,
            octet_agent::ExtensionRuntimeConfig::new(temp.path()),
        )
        .await
    ...
    async fn generated_link_runs_the_pinned_real_pi_hello_example_when_selected() {
        let Some(pi_package) = std::env::var_os("OCTET_PI_REAL_PACKAGE").map(PathBuf::from) else {
            return;
        };
        if std::process::Command::new("node")
            .arg("--version")
        ...
        if !source.is_file() {
            panic!("OCTET_PI_REAL_PACKAGE does not contain examples/extensions/hello.ts");
        }
        let temp = tempfile::tempdir().unwrap();
        let extension_root = temp.path().join("extensions");
        install(
        ...
        let output = process
            .call_tool(
                "hello",
                serde_json::json!({"name": "octet"}),
                process.current_context(),
            )
            .await
            .unwrap();
        assert!(output.content.contains("Hello, octet!"));
        assert!(process.shutdown().await);
    }
    ...
    async fn generated_aggregate_runs_pinned_real_pi_examples_when_selected() {
        ...
        if !hello.is_file() || !plan_mode.is_file() {
            panic!(
                "OCTET_PI_REAL_PACKAGE does not contain the reviewed hello and plan-mode examples"
            );
        }
        let temp = tempfile::tempdir().unwrap();
        let extension_root = temp.path().join("extensions");
