...
impl HostProcess {
    fn spawn(home: &std::path::Path, workspace: &std::path::Path) -> Self {
        let mut command = Command::new(env!("CARGO_BIN_EXE_octet-host"));
        command
        ...
        let mut child = command.spawn().expect("spawn octet-host");
    ...
    fn recv(&self) -> Value {
        let line = self
            ...
            .expect("timed out waiting for octet-host");
    ...
    fn wait(&mut self) -> std::process::ExitStatus {
        ...
        loop {
            ...
            assert!(Instant::now() < deadline, "octet-host did not exit");
...
fn run_request(
    ...
    let mut request = json!({
        ...
        "base_url": base_url,
        "api_key": "test-only",
        "custom_headers": {"x-octet-host-test": "present"},
...
fn inline_provider_run_streams_and_resumes_a_native_session() {
    ...
    let session = octet_agent::Session::open_read_only(&session_file).expect("reopen host session");
    ...
    for request in [&first_provider_request, &second_provider_request] {
        ...
        assert!(head.contains("x-octet-host-test: present"));
...
"#,
    ...
    assert!(
        ...
        "Controlled octet-host launched an executable extension"
    );
