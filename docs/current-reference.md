...
- [Sessions](#durable-branchable-sessions)
- [Interactive commands](#interactive-command-reference)
- [Configuration and CLI](#configuration)
- [Prompts, skills, and extensions](#filesystem-native-customization)
...
Codex routes that advertise Responses Lite use that codec contract for both
ordinary and native compact requests. The codec sends the Lite header, places
...
local replay; native Responses mode uses the persisted route-affine opaque
replay rather than depending on a surviving server response ID. A mismatch
also uses full replay instead of guessing a continuation.
...
See [docs/sessions.md](sessions.md) for the record schema, branch semantics, redaction contract, and recovery behavior.
...
mode = "local"
threshold_fraction = 1.0
# Codex request budgeting is capped at 272K by default. Set a smaller value
# to constrain the active working set; zero is equivalent to unsetting.
...
Start with [examples/README.md](../examples/README.md), then read [docs/resources.md](resources.md) and [docs/extensions.md](extensions.md).
...
[documented in docs/build-profiles.md](build-profiles.md).
...
## Historical Ygg distribution reference

**Ygg 0.6.x only; not an octet 0.7.0 installation or update guide.** The retained
commands and channel contracts below document older source/release behavior.
They may install an older Ygg release and may alter an existing Ygg installation.
Do not run them to obtain octet, infer that a channel is currently published, or
use the old hotfix migration as a Ygg-to-octet migration promise. For current
checkout builds use [the root README](../README.md#build-this-checkout); for
historical release context see [v0.6.7](releases/v0.6.7.md).
...
See [`docs/release/npm-trusted-publishing.md`](release/npm-trusted-publishing.md)
...
[`docs/distribution.md`](distribution.md) for channel and release-gate
details.
...
[methodology, artifact hashes, exclusions, and reproduction guide](benchmarks/tb21-v0.6.2/README.md).
...
These are historical Ygg v0.6.2 results, not octet 0.7.0 measurements.
