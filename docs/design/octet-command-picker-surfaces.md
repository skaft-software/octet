...
[`octet-presentation.md`](octet-presentation.md) and the terminal mechanics in
[`octet-tui.md`](octet-tui.md); it does not introduce a second renderer, theme, or
modal system.
...
- Approval panels remain enforcement surfaces. They retain the separate
  no-filter/no-count and visible-selected-action rules in `octet-tui.md`; this
...
`crates/octet-coding-agent/src/tui/view/ordinary_surface_contract_tests.rs`. It
