...
**Unimplemented design reference.** The octet 0.7.6 source supports only
...
[exact 0.7.6 GitHub release](https://github.com/skaft-software/octet/releases/tag/v0.7.6)
...
pairing, not peer replication. One `octet serve` process owns the session catalog,
...
- opt-in LAN listening with `octet serve --lan`;
...
- authenticated HTTP and WebSocket access to the existing octet Serve protocol;
...
- any implicit expansion of octet tool authority. In particular, pairing does not
...
```text
service type: _octet._tcp.local.
TXT:
  v=1
  id=<host-id>
...
Apple clients declare `_octet._tcp` in `NSBonjourServices` and provide
...
```text
Authorization: octet-Device v1:<device-id>:<base64url-secret>
```
...
LAN transport authenticates them with `octet-Device`.
...
The only unavoidable `octet-coding-agent` seams are:
...
- `Cargo.toml`: retain `octet-serve-backend` behind the single optional `serve`
  feature.
...
xcodebuild test -project apps/apple/octet.xcodeproj -scheme octet -destination 'platform=macOS'
xcodebuild test -project apps/apple/octet.xcodeproj -scheme octet -destination 'platform=iOS Simulator,name=iPhone 17'
