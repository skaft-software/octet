...
by those receipts**. The wave-15 surfaces (deferred/durable lifecycle, codec
declarations, request hooks and proxy transports, image generation, scoped
models and local shell escapes, the single-surface subagent roster) are also
unqualified: the wave's own bounded compile checks end 101 and the wave-15
verification suite has not been run
(see [VERIFICATION.md](VERIFICATION.md#wave-15-usage-defects-roster-repair-and-integration-seams)).
No all-green current-worktree or full-workspace-test claim.
...
**Current overlay (wave-15 reconciliation, 2026-09-16):** of the 88 rows,
31 are `Verified`, 16 `Landed`, 13 `Implemented`, 21 `Partial`, 1 `Blocked`,
4 `Withdrawn` and 2 `Excluded`; none remain `Unverified` or `Pending`, because
each row now names either its verified artifact, its landed/implemented source
path, or its exact missing primitive. `Implemented` means inspected source with
behavioral verification still pending: no wave-15 Rust compile or test run has
passed, and no row was upgraded on a worker's word (see
...
| --- | --- | --- | --- |
| 1a.1 | baseten, qwen-token-plan, qwen-token-plan-cn, qwen-token-plan-individual, zai-coding-cn declarations/catalog/tested #252 rows | providers | Partial; five provider declarations re-verified against the reference with a public-SDK boundary test (`tests/provider_contract.rs::token_plan_and_coding_presets_are_public_credential_free_definitions`) and octet→models.dev membership gates in the generator (the owner reports `scripts/test_refresh_models_dev_metadata.py` 6 passed and the scripts suite 58 passed). The pinned static catalogs are still not materialised: `scripts/refresh-models-dev-pricing.py` needs its network refresh before these providers get pinned names/limits/modalities/pricing |
| 1a.2 | radius + pi-messages codec + dynamic discovery | codec depth | Blocked; the `pi-messages` request/SSE codec (`crates/octet-ai/src/protocol/pi_messages.rs`, authored unit tests plus three wire fixtures) and bounded, credential-free Radius `/v1/config` discovery (`declarations/radius.rs`) now exist, but no route can select them: `Protocol::PiMessages` is absent from `types.rs`, `client.rs` has no build/decode/EOF arm, and no declaration/`build.rs` arm registers the gateway, so the codec is unreachable (authored tests unexecuted) |
| 1b.1 | Per-request apiKey, headers, env, fetch, onPayload, onResponse, timeoutMs, maxRetries, maxRetryDelayMs, metadata, transformHeaders | providers | Partial; request-local `RequestOverrides` (headers, env, timeout) and Azure destination/API-version options are consumed on the wire, and the octet-ai HTTP path now consumes per-request `api_key`, `transform_headers`, `on_payload`, `on_response` and `fetch` (`client.rs::stream_with_host_options`, `runtime.rs`; authored `tests/runtime_hooks.rs`, unexecuted). A non-empty `metadata` map is still refused until a codec wire field exists, retries stay host-owned, and no product surface for the hooks is claimed |
| 1b.2 | Model/request samplingParams and model headers | providers | Partial; model preset sampling params and headers now merge into the wire with an explicit allowlist (legacy `functions`/`function_call`, hosted web search and stored-prompt keys are refused, and caller `stop` wins), request temperature precedence is covered, and header secrecy/signing order is tested; per-model thinking/priority emission for the remaining providers is still outstanding. Discovery now attaches declaration-owned wire profiles for baseten/qwen-token-plan*/zai-coding-cn (`providers/models.rs::discovery_wire_preset`, `providers/catalog.rs::discovered_wire_preset`; authored tests), so those five routes gain the reference's thinking format and reasoning control (unexecuted) |
| 1b.3 | HTTP_PROXY / HTTPS_PROXY / ALL_PROXY / NO_PROXY root and subdomain semantics | providers | Implemented; the resolver pre-existed; authored `tests/proxy_transport.rs` now exercises absolute-form requests through a real loopback proxy, `NO_PROXY` plus root/subdomain exclusion with zero proxy contact, fail-closed malformed/dead proxies and a proxied WebSocket-preferred Responses route over HTTP (unexecuted). Native TLS-over-CONNECT WebSocket tunnelling is not implemented; WS-preferred endpoints deliberately use the proxied HTTP/SSE route |
...
| 1b.5 | vllmPriority, supportsMaxOutputTokens, thinkingTokenBudgetField, chatTemplateArgs/Kwargs, $var interpolation, string thinking | providers | Partial; the codec emission exists and is consumed by the OpenAI Chat/Responses builders (`protocol/preset.rs` emits `vllm_priority`, `chat_template_args`/`kwargs`, `thinking_token_budget_field` and the thinking formats, with `$var` interpolation and fail-closed unknown names; `supports_max_output_tokens` is consumed by catalog validation), and discovery now produces presets for baseten/qwen-token-plan*/zai-coding-cn (1b.2). The remaining gap is the pinned per-model catalog in the generated declaration schema, which the other providers still need |
| 1b.6 | ANTHROPIC_AUTH_TOKEN, ANTHROPIC_OAUTH_TOKEN, GOOGLE_CLOUD_API_KEY aliases | providers | Implemented; bearer-alias precedence reads the shared `ANTHROPIC_BEARER_TOKEN_VARIABLES` list, and `GOOGLE_CLOUD_API_KEY` now selects the vertex API-key authority in both crates (`octet-ai/src/auth.rs::select_vertex_credential`, `providers/vertex.rs`, bootstrap readiness/registration; authored tests unexecuted). The declared SDK/provider shape still describes ADC only (`AuthenticationSpec::ApiKeyOrApplicationDefaultCredentials` absent from `build.rs`/`declarations.json`) |
| 1c.1 | Strict JSON-schema and grammar/Lark/regex custom tools across five codec families; strict-prefer defaults | codec depth | Partial; explicit declarations emit, and custom-call decode/replay plus per-route strict/grammar defaults are implemented (`provider_parity.rs::grammar_*`, `cross_protocol_tests::{constrained_sampling_wire_shape_across_codecs,strict_and_grammar_tool_support_are_per_route_declared_defaults}`). The reference's provider-identity strict default (`isMoonshot`/`isTogether`/… disable it) has no local codec equivalent, so a provider must set `supports_strict_mode: Some(false)` in its preset (data, not codec) |
...
| 1c.3 | Anthropic OAuth/fine-grained/interleaved betas, mid-conversation effort, caller-beta merge | codec depth | Partial; declared Anthropic compat, eager tool streaming with the legacy beta when disabled, inferred server-side-fallback/mid-conversation-effort beta pairs, declared fallback lists and the empty-signature replay policy landed with five authored codec tests; fallback-model pricing and the refusal `errorMessage` still need `stream.rs::ResponseBuilder::finish_mut` |
| 1c.4 | Anthropic refusal fallbacks, blocks and fallback-model pricing | codec depth | Partial; refusal `stop_details.explanation` is recorded as a bounded `anthropic_refusal` diagnostic and fallbacks emit only from a declared, validated list; fallback-model pricing (terminal model ≠ requested, matched against `AnthropicFallbackCost`) and the refusal `errorMessage` remain absent on the streaming path |
| 1c.5 | Bedrock profile ARN regions/application profiles/web identity/bearer token | codec depth | Partial; bearer/web identity present, and pure ARN/application-inference-profile region resolution landed in `declarations/bedrock.rs` (reference precedence, `bedrock-runtime[-fips].<region>` extraction, standard-endpoint rebuild; authored unit + declaration tests). `providers/auth.rs::aws_bedrock_region()` and bootstrap still do not pass the selected model id or consume the resolver, so the host wiring is missing |
| 1c.6 | Codex per-request sse/websocket/websocket-cached/auto, connect deadline and debug stats | codec depth | Partial; host retry/envelope regressions pass; declaration-owned per-request transport resolution (sse/websocket/websocket-cached/auto), the 15 s connect deadline and bounded debug-stats counters landed in `declarations/codex.rs` with `RequestOverrides` fields and authored tests; `client.rs`/`responses_ws.rs` still do not consume the override, apply the deadline or expose the stats |
...
| 1c.8 | Mistral Conversations reasoning_effort | codec depth | Partial; `ModelPreset::mistral_reasoning` is admitted for `Protocol::MistralConversations` and the codec emits exactly one native control (`completion_args.reasoning_effort` via the level map with the reference's `"high"` fallback, or `prompt_mode="reasoning"`), failing closed for undeclared/budget reasoning with authored unit tests. `catalog.rs:403` still rejects every reasoning capability on this protocol, so no live route can declare the profile |
...
| 1c.10 | rawStopReason, responseModel, providerThinkingLevel, diagnostics and per-tool-result usage | codec depth | Partial; `pi_messages` carries `providerThinkingLevel`/`rewrite` as bounded diagnostics and Anthropic records the refusal diagnostic; the agent now folds `ToolOutput::usage` into billed run totals without inflating turn context (`tests/tool_result_usage.rs`, authored). The typed fields are still absent: `AssistantMessage.{response_model,provider_thinking_level,raw_stop_reason,diagnostics}` and `octet_ai::ToolResult.usage` do not exist, so per-tool usage is not durable and raw stop/model/thinking values are dropped |
| 1d.1 | apiKey.check/resolve, oauth.login/refresh/logout, AuthCheck, minOAuthValidityMs, isSubscription | auth depth | Partial; the vertex API-key resolve half now selects `GOOGLE_CLOUD_API_KEY` and bearer aliases read the shared list (1b.6); the explicit host `check`/`resolve`/`minOAuthValidityMs` policy surface remains missing (`octet-agent/src/extension_provider.rs` owner), and brokered OAuth policy expansion stays excluded |
...
| 1e.1 | Faux provider pending/ready/failed/cancelled deferred handles + deferred stop reason | streaming | Implemented; `octet_ai::faux` scripts pending/ready/failed/cancelled deferred handles with counters, `StopReason::Deferred` + `Response.deferred` + `CanonicalStreamAssembler::set_deferred` exist, and the agent classifies/persists the declaration and reports `run_suspend`/`run_resume` with a telemetry span. Authored `tests/deferred.rs` (6) and `tests/deferred_runs.rs` (6) are unexecuted; a settled poll is not yet committed as the assistant turn |
...
| 1e.3 | Image generation API, OpenRouter adapter, generated image-model catalog + generator | streaming | Implemented; `octet_ai::images` implements the OpenRouter adapter (modalities, inline images, bounded reads, retry-after, usage/cost split, `data:`-only decode, ≤8 images, cancellation, in-band reporting) with an embedded 11-route catalog generated from a pinned source record; the generator's own unittests pass (5 passed, owner receipt) and `--check` is byte-identical, while the Rust adapter/API tests (`src/images.rs` 8 units, `tests/images.rs` 5) are authored and unexecuted. Live OpenRouter image acceptance and any cost ceiling remain external gates |
| 2a.1 | Alternate screen, autowrap, fixed viewport, final document to main screen | TUI | Implemented; `sexy-tui-rs` now has a public `AlternateScreen` (enter disables autowrap and clears the fixed viewport, absolute-row paint with differential rows and cursor handling, exit restores the final document to the main screen, synchronized output) wired as an explicit opt-in (`OCTET_TUI_ALT_SCREEN`); the pinned `primary-*.trace` goldens deliberately keep the default primary screen, and the authored tests are unexecuted |
| 2a.2 | Constrained layout root/VStack/HStack/ScrollView sizing/visibility/nested scrolling/last-frame hit-testing | TUI | Implemented; public `sexy-tui-rs::layout` ports the constrained root/VStack/HStack/ScrollView (basis/clamp/grow-shrink allocation, visibility, nested clips/scrollbar geometry, last-frame hit-testing) with authored tests; the octet shell's own transcript viewport still does not consume a layout frame, and nothing is compiled/executed |
| 2a.3 | Alt-wheel ×5 and unused-delta chain/contain | TUI | Implemented; product Alt-wheel ×5 (3+15) matches the reference, and the library now implements the unused-delta chain/contain (`route_wheel_delta`, `wheel_scroll_lines`, primary-scroll-view absorption) with authored tests; unexecuted |
...
| 2a.5 | Clickable jump to latest | TUI | Partial; the existing scroll indicator is a clickable return-to-live affordance when history is scrolled, while the clickable-jump mechanism of the alternate-screen surface is now an opt-in library capability (2a.1) and is not wired to this affordance, so this is not the upstream mechanism |
...
| 2c.5 | Component mouse events/MouseRegion/capture/focus/hover/click selection/link hit-test/right paste | TUI | Implemented; public `sexy-tui-rs::mouse` routes events with `MouseRegion` forwarding, capture, click synthesis/double-click, focus/hover, wheel chaining through last-frame layout, OSC 8 link reporting and the right-click-paste fallback, and the product adds `transcript_link_at_screen_cell` link hit-testing with an authored test; link activation, scrollbar drag and overlay routing remain product-owned; unexecuted |
...
| 2d.1 | /settings defaults/theme/transport/images/editor padding | TUI | Partial; `/settings` is a built-in command (exact-name parse, popup/help) that reports and changes defaults, theme, images and default model/reasoning, delegates the theme to the existing terminal theme service, persists images as a TOML boolean and explicitly refuses `default trust …`; transport and editor padding are reported as route/theme facts instead of a fake switch, so both remain read-only. Authored command/product/PTY tests are unexecuted |
| 2d.2 | /scoped-models all/clear/provider toggle/reorder/persistence/cycling | TUI | Implemented; `/scoped-models all|clear|enable|disable|toggle|move` mutates the live Ctrl+P scope, persists the exact ordered pattern list (user `models` key), warns (does not fail) on an invalid hand edit and is interactive-only; a project layer cannot override it. An empty scope persists as a cleared key and reopens as unrestricted — documented, not hidden. Authored tests are unexecuted |
| 2d.3 | /hotkeys | TUI | Verified for the executed cases; `/hotkeys` is a built-in command rendered from the resolved binding set |
| 2d.4 | /debug rendered lines and message JSON | TUI | Verified; hidden command (exact-name only, absent from the popup and `/help`) writes an owner-private `~/.octet/octet-debug.log` with the terminal size, every rendered line plus its visible width, and the agent messages as JSONL, then reports the path. Coverage: `commands::tests::debug_is_hidden_but_parses_and_reports_every_rendered_line` and `tui::view::renderer_runtime::scheduler_tests::diagnostics_read_the_retained_frame_without_scheduling_a_repaint` inside the 1526-test coding library receipt |
...
| 2d.9 | Model cycling forward/back and selector hotkeys | TUI | Implemented; forward/back cycling follows the App-supplied scope order, de-duplicates while preserving it, advances from the last queued target while active and re-applies each scope entry's `:level` suffix on a model reconfig (`app/mod.rs::apply_reconfig`); selector bindings and the ordered `--models`/`/scoped-models` scope feed it (authored tests unexecuted; scope persistence is deliberately not included) |
...
| 2d.11 | !command / !!command context exclusion | TUI | Implemented; `!command`/`!!command` parse exactly, run through the product's bounded local-shell capture and the ordinary `EffectBroker` process approval, work idle and mid-run, and record `!` as a model-visible user message while `!!` is a non-model-visible config entry marked `[excluded from model context]`; an unresolved tool call keeps the excluded form so a call is never split from its result. Authored parser/dispatch/product tests are unexecuted; the local escape does not re-implement the model tool's `--shell-path` resolution and has no guard against overlapping a running model bash call |
...
| 4.6 | Opt-in PowerShell, Windows CI evidence | tools | Partial; the coding product now adds `powershell` to the allowlist through `--powershell` (`config.rs::ToolPolicy::include`, conflicts with `--tools`/`--no-tools`, inert-host warning) with authored unit/integration tests; the tool registration remains `#[cfg(windows)]`, so Windows execution evidence is still hardware-gated (unexecuted here) |
...
| 4.12 | Deferred provider suspend/resume/handles/poll permits | tools | Partial; the transport half landed (`fetch_deferred` consumes a generation-bound permit before any provider work, with typed `NoPermit`/`AlreadyConsumed`/`StaleGeneration` refusals; authored `tests/deferred.rs`) and the kernel half adds a durable, generation-fenced suspended/effect-pending store, session record/replay, run-loop park and resume/cancel API (`tools/deferred.rs`, `session.rs`, `agent.rs`; authored `tests/deferred_runs.rs`, unexecuted). Still missing: the coding-agent session mirror does not accept the `deferred_run` record, and a parked run ends as `FinishReason::Failed(DeferredSuspended)` with no honest frontend terminal state |
...
| 5.7 | --models glob cycling constraints | CLI | Implemented; `select_scoped_models` resolves a literal `provider/model`/bare-id reference exactly before glob matching, preserves the requested pattern order, de-duplicates first-seen and keeps each `:level` suffix (`cli/parity.rs::model_pattern_matches`/`exact_model_reference`; authored unit + `parity_cli` tests); the persisted `/scoped-models` list is interactive-only |
