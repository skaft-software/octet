...
vendored. Implementations live in octet's own crates.

**Evidence scope:** existing Verified labels describe the prior delivery receipts,
not blanket qualification of the current dirty 0.8.0 worktree. Newly changed
consumers are explicitly pending fresh tests below.
...
- `crates/octet-coding-agent/src/cli/eval.rs` — `octet eval run` (row 5.11): an
  isolated harness with a safe scripted default, explicit private loopback model
  profiles, bounded case supervision and uncertainty-aware measurement reports.
...
| 5.8 single-file HTML export | Verified for the executed cases | Script-free light/dark document with parsed Markdown, role/tool/reasoning displays, parent navigation, a first-256-message index and complete raw records. Raw author HTML and external destinations are inert. Only bounded, decoded and MIME-matched inline raster images are active. Markdown above 512 KiB or nesting 64 falls back to complete literal text; golden/security/real-session tests execute in the coding library receipt, and the process-level `parity_cli` export test passes with redaction-before-projection, typed-media and owner-private destination checks. Browser static inspection and a complete checked-in document golden remain outstanding. |
...
| 5.11 model-backed eval harness | Verified for the harness boundary; no live-provider run | Safe scripted fixture remains the default. Explicit `--model-profile` admits an owner-private bounded JSON profile for an independently running literal-loopback HTTP model, with no ambient credentials or redirects. Each case uses private disposable HOME/workspace/session roots, no tools, literal stdin, token/wall-time/output bounds and pre-request known-price cost admission. Unknown prices or missing usage remain uncertain, including failed calls. Reports distinguish scripted/model backends and exact/uncertain deltas. Ten process-boundary tests, including an independent endpoint and budget/failure/isolation cases, pass in `parity-next-coding-all-07`. Scripted-fixture evaluation is still the default and no paid/live model was invoked; direct remote/live-provider qualification is not claimed; [operator contract](../cli.md#local-evaluation). |
...
- `octet eval run <SUITE>` defaults to an isolated scripted fixture. Explicit
  `--model-profile` selects a private local-server profile for real runtime-backed
  inference; private measurements and `--baseline` deltas preserve uncertainty.
  A local server may itself route to paid services; no remote-provider or live
  billing qualification is implied.
