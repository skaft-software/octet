...
octet has two process boundaries for consumers:

- Rust applications embed the public `octet-agent` and `octet-ai` crates. The
  `octet_sdk` library in `octet-coding-agent` contains the product runtime shared by
  the `octet` and `octet-host` binaries.
- Other languages launch `octet-host` and exchange one UTF-8 JSON object per line
...
Cargo installs embed octet's text documentation in the binaries. On first use,
octet materializes it under `${CARGO_HOME:-$HOME/.cargo}/share/octet`; the managed
copy is refreshed when the Cargo-channel `octet update` installs a newer release.
...
from `octet --login`, environment/configuration provider setup, and the NDJSON
`octet-host` protocol: those surfaces cannot safely own the host's GitHub OAuth
state. A standalone octet catalog therefore never advertises Copilot models.

A Rust embedding application implements `octet_sdk::provider::CopilotHost`, owns
...
| `session_dir` | Application-owned session root; defaults to `<workspace>/.octet/sessions`. |
...
| `tools` | Explicit tool registration allowlist. `[]` disables tools; omission uses octet's default registered surface. Registration does not bypass effect admission. |
...
| `reasoning` | octet reasoning level accepted by the selected model. |
...
An application can define one route without modifying global octet configuration:
...
Without `base_url`, `model` is resolved through octet's normal catalog and
credential resolvers. If octet has no Codex credential on first use, it can copy a
valid existing third-party Codex CLI credential from `~/.codex/auth.json`. It
does not import earlier-first-party Hamr/Ygg stores. The imported credential is
written to `~/.octet/credentials/codex.json` with owner-only permissions while
holding octet's cross-process refresh lock. Source credentials are never modified
or deleted.
...
`octet-host` never waits for interactive input. It always uses the Controlled
...
contract explicit. On Unix, `octet-host` coordinates `HUP`, `INT`, `QUIT`, and
...
The application chooses `workspace` and `session_dir`. octet loads its normal
deterministic resource layers (`~/.octet`, trusted workspace `.octet`, then explicit
...
`system_prompt`; use octet sessions for model/tool continuity.
