...
- A non-timeout connection-establishment failure is different from an ambiguous accepted request. Sending a POST, awaiting headers, or losing its body can leave execution indeterminate. No visible text does **not** make replay safe; body disconnects remain terminal at the agent layer, including before output. Provisional TUI removal is not replay permission. Deterministic regressions do not qualify live recovery or the renamed candidate.
